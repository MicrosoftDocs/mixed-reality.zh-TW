...
9.  在 [*函數應用程式*] 儀表板上，將*滑鼠游標暫*留在左側面板中的 [函式] 上方，然後按一下 **+ （加號）** 符號。
...
> [!NOTE]
> 在此課程中，您將**不**需要有動作控制器。 如果您需要支援設定沉浸式耳機，請[流覽混合現實設定一文](https://support.microsoft.com/help/4043101/windows-10-set-up-windows-mixed-reality)。
...
6.  接下來，您將建立**3D 文字**物件，以提供 Azure 服務狀態的意見反應。
    ...
    再以滑鼠右鍵按一下 [階層] 面板中的 [ **GazeButton** ]，並將**3D 物件** > **3d 文字**物件當做*子*系。
...
9.  變更**文本網格**元件以符合下列內容：
    ...
    > [!TIP]
    > 這裡選取的色彩是十六進位色彩： **000000FF**，不過您可以自行選擇，只是要確定它是可讀取的。
...
1.  使用 **資產** > 匯**入封裝** > **自訂封裝** 功能表選項，將**unitypackage**檔案新增至 Unity。

2.  在快顯的 [匯**入 Unity 封裝**] 方塊中，您可以選取 [**外掛程式**] > [**儲存體**] 底下的所有專案。 取消核取其他所有專案，因為這個課程並不需要。

    ![匯入至封裝](images/AzureLabs-Lab5-45.png)

3.  按一下 [匯**入**] 按鈕，將專案新增至您的專案。

4.  移至 [*外掛程式*] 底下的 [*儲存體*] 資料夾，然後在 [專案] 視圖中選取*下列外掛程式：*
...
1.  在 專案 面板中，以滑鼠右鍵按一下 *資產* 資料夾，然後**建立** > **資料夾**。 將資料夾命名為**Scripts**。
...
備妥其他腳本之後，現在就可以*完成* *AzureServices*類別。 這將透過下列途徑達成：
...
5.  新增方法來抓取儲存在*Azure 儲存體*檔案中的檔案中的文字，並將*它還原*序列化為清單。
...
3.  在 [**解決方案**設定] 中，選取 [ **Debug**]。
    ...
    > - 請確定**開發人員模式**已**開啟**;在 [**設定**] 中找到 > **為開發人員** **更新 & 安全性** > 。
