...
**剪輯平面**為了達到最大的緩和度，我們建議您在85cm 時使用 fadeout 的內容，從1m 開始裁剪轉譯距離。 在全息影像和使用者都是固定的全息影像的應用程式中，可以像50cm 一樣輕鬆地觀看。 在這些情況下，應用程式應該將剪輯平面放在靠近30cm 的位置，淡出至少應該從剪輯平面開始10cm。 每當內容接近85cm 時，請務必確保使用者不會經常從全息影像移近或更遠，或不常從使用者處移動全息影像，因為這些情況很可能會造成 discomfort 的問題。vergence-住宿衝突。 內容的設計目的是要將互動的需求降到最低，而不是從使用者85cm，但當內容必須比85cm 更接近時，開發人員的最佳規則就是設計案例，讓使用者和/或全息影像不會在超過25% 的深度移動。
