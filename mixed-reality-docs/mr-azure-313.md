...
<span data-ttu-id="dca2d-149">下列的硬體和軟體是必要項目：</span><span class="sxs-lookup"><span data-stu-id="dca2d-149">The following hardware and software is required:</span></span>
...
<span data-ttu-id="dca2d-166">校正的說明，請遵循此[HoloLens 校正文章連結](calibration.md#hololens)。</span><span class="sxs-lookup"><span data-stu-id="dca2d-166">For help on Calibration, please follow this [link to the HoloLens Calibration article](calibration.md#hololens).</span></span>
...
<span data-ttu-id="dca2d-181">不過，您可以遵循下列步驟，如果您想要使用您自己的自訂視覺模型：</span><span class="sxs-lookup"><span data-stu-id="dca2d-181">However, you can follow these steps if you wish to use your own Custom Vision model:</span></span>
...
2. <span data-ttu-id="dca2d-199">移至**建立資源**並尋找**Container Registry**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-199">Go to **Create a resource** and look for **Container Registry**.</span></span>
...
5.  <span data-ttu-id="dca2d-232">在頁面底部按一下**下一步:大小和縮放比例**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-232">On the bottom of the page click on **Next: Size and scale**.</span></span>
...
14. <span data-ttu-id="dca2d-252">在 *裝置詳細資料*頁面隨即出現，需要一份**連接字串**（主索引鍵）。</span><span class="sxs-lookup"><span data-stu-id="dca2d-252">On the *Device Details* page that appears, take a copy of the **Connection String** (primary key).</span></span>
...
3.  <span data-ttu-id="dca2d-266">[Visual Studio Code (也稱為 VS Code)](https://code.visualstudio.com/download)。</span><span class="sxs-lookup"><span data-stu-id="dca2d-266">[Visual Studio Code (also known as VS Code)](https://code.visualstudio.com/download).</span></span>
...
1. <span data-ttu-id="dca2d-303">開啟**VS Code**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-303">Open **VS Code**.</span></span>
...
7. <span data-ttu-id="dca2d-322">一次**Cookiecutter**已完成安裝，您應該重新啟動您的電腦，以便**Cookiecutter**可辨識的命令，您的系統環境中。</span><span class="sxs-lookup"><span data-stu-id="dca2d-322">Once **Cookiecutter** has finished installing, you should restart your machine, so that **Cookiecutter** is recognized as a command, within your System's environment.</span></span>
...
1. <span data-ttu-id="dca2d-357">使用 VS Code 開啟、 瀏覽至您的模組資料夾，並尋找呼叫的指令碼**主要<span></span>.py**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-357">With VS Code open, navigate to your module folder, and look for the script called **main<span></span>.py**.</span></span> <span data-ttu-id="dca2d-358">按兩下以開啟它。</span><span class="sxs-lookup"><span data-stu-id="dca2d-358">Double-click to open it.</span></span>
...
9. <span data-ttu-id="dca2d-405">若要檢視從您的裝置傳送到 IoT 中樞的訊息，再按一下滑鼠右鍵在您的裝置名稱，在**Azure IoT 中樞裝置**區段中**總管**] 面板，然後按一下 [**開始監視D2C 訊息**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-405">To view the messages sent from your device to the IoT Hub, right-click again on your device name in the **Azure IoT Hub Devices** section, in the **Explorer** panel, and click on **Start Monitoring D2C Message**.</span></span> <span data-ttu-id="dca2d-406">從您的裝置所傳送的訊息應該會出現在 VS 終端機中。</span><span class="sxs-lookup"><span data-stu-id="dca2d-406">The messages sent from your device should appear in the VS Terminal.</span></span> <span data-ttu-id="dca2d-407">請耐心等候，因為這可能需要一些時間。</span><span class="sxs-lookup"><span data-stu-id="dca2d-407">Be patient, as this may take some time.</span></span> <span data-ttu-id="dca2d-408">請參閱下一章中的偵錯，以及檢查部署是否成功。</span><span class="sxs-lookup"><span data-stu-id="dca2d-408">See the next Chapter for debugging, and checking if deployment was successful.</span></span>

<span data-ttu-id="dca2d-409">此模組中的映像之間會現在逐一查看**映像**資料夾和分析系統，與每個反覆項目。</span><span class="sxs-lookup"><span data-stu-id="dca2d-409">This module will now iterate between the images in the **images** folder and analyze them, with each iteration.</span></span> <span data-ttu-id="dca2d-410">這是明顯只是示範如何取得基本的機器學習服務模型在 IoT Edge 裝置環境中工作。</span><span class="sxs-lookup"><span data-stu-id="dca2d-410">This is obviously just a demonstration of how to get the basic machine learning model to work in an IoT Edge device environment.</span></span> 

<span data-ttu-id="dca2d-411">若要擴充此範例的功能，您可以繼續透過數種方式。</span><span class="sxs-lookup"><span data-stu-id="dca2d-411">To expand the functionality of this example, you could proceed in several ways.</span></span> <span data-ttu-id="dca2d-412">一種方法可以包括一些程式碼在容器中，會擷取從網路攝影機，連線到裝置，並將影像儲存映像資料夾中的相片。</span><span class="sxs-lookup"><span data-stu-id="dca2d-412">One way could be including some code in the container, that captures photos from a webcam that is connected to the device, and saves the images in the images folder.</span></span> 

<span data-ttu-id="dca2d-413">另一種方式可能複製映像從 IoT 裝置加入至容器。</span><span class="sxs-lookup"><span data-stu-id="dca2d-413">Another way could be copying the images from the IoT device into the container.</span></span> <span data-ttu-id="dca2d-414">實用的方法是在 IoT 裝置 （如果您想要自動化程序，或許是小型的應用程式無法執行作業） 的終端機中執行下列命令。</span><span class="sxs-lookup"><span data-stu-id="dca2d-414">A practical way to do that is to run the following command in the IoT device Terminal (perhaps a small app could do the job, if you wished to automate the process).</span></span> <span data-ttu-id="dca2d-415">您可以從您的檔案儲存所在的資料夾位置以手動方式執行來測試此命令：</span><span class="sxs-lookup"><span data-stu-id="dca2d-415">You can test this command by running it manually from the folder location where your files are stored:</span></span>

```bash
    sudo docker cp <filename> <modulename>:/app/images/<a name of your choice>
```

## <a name="chapter-10---debugging-the-iot-edge-runtime"></a><span data-ttu-id="dca2d-416">第 10 章-偵錯 IoT Edge 執行階段</span><span class="sxs-lookup"><span data-stu-id="dca2d-416">Chapter 10 - Debugging the IoT Edge Runtime</span></span>

<span data-ttu-id="dca2d-417">以下是命令列和秘訣，可協助您監視和偵錯的傳訊活動的清單*IoT Edge 執行階段*，從您**Ubuntu 裝置**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-417">The following are a list of command lines, and tips, to help you monitor and debug the messaging activity of the *IoT Edge Runtime*, from your **Ubuntu device**.</span></span> 

- <span data-ttu-id="dca2d-418">請檢查*IoT Edge 執行階段*藉由執行下列命令列的狀態：</span><span class="sxs-lookup"><span data-stu-id="dca2d-418">Check the *IoT Edge Runtime* status by running the following command line:</span></span>
    ...
    > [!NOTE]
    > <span data-ttu-id="dca2d-419">按下時，請記得**Ctrl + C**，以完成檢視狀態。</span><span class="sxs-lookup"><span data-stu-id="dca2d-419">Remember to press **Ctrl + C**, to finish viewing the status.</span></span>

- <span data-ttu-id="dca2d-420">列出目前已部署的容器。</span><span class="sxs-lookup"><span data-stu-id="dca2d-420">List the containers that are currently deployed.</span></span> <span data-ttu-id="dca2d-421">如果*IoT 中樞服務*有容器成功部署，則會顯示藉由執行下列命令列：</span><span class="sxs-lookup"><span data-stu-id="dca2d-421">If the *IoT Hub Service* has deployed the containers successfully, they will be displayed by running the following command line:</span></span>

    ```bash
        sudo iotedge list
    ```

    <span data-ttu-id="dca2d-422">或</span><span class="sxs-lookup"><span data-stu-id="dca2d-422">Or</span></span>
    ...
    > [!NOTE]
    > <span data-ttu-id="dca2d-423">以上是檢查是否您的模組是否已成功部署，就會出現在清單中的好方法否則，您將**僅**請參閱*edgeHub*並*edgeAgent*。</span><span class="sxs-lookup"><span data-stu-id="dca2d-423">The above is a good way to check whether your module has been deployed successfully, as it will appear in the list; you will otherwise **only** see the *edgeHub* and *edgeAgent*.</span></span>

- <span data-ttu-id="dca2d-424">若要顯示容器的程式碼記錄檔，請執行下列命令列：</span><span class="sxs-lookup"><span data-stu-id="dca2d-424">To display the code logs of a container, run the following command line:</span></span>

    ```bash
        journalctl -u iotedge
    ```

<span data-ttu-id="dca2d-425">**用來管理 IoT Edge 執行階段的有用命令：**</span><span class="sxs-lookup"><span data-stu-id="dca2d-425">**Useful commands to manage the IoT Edge Runtime:**</span></span>

-  <span data-ttu-id="dca2d-426">若要刪除主應用程式中的所有容器：</span><span class="sxs-lookup"><span data-stu-id="dca2d-426">To delete all containers in the host:</span></span>

    ```bash
        sudo docker rm -f $(sudo docker ps -aq)
    ```

-  <span data-ttu-id="dca2d-427">若要停止*IoT Edge 執行階段*:</span><span class="sxs-lookup"><span data-stu-id="dca2d-427">To stop the *IoT Edge Runtime*:</span></span>

    ```bash
        sudo systemctl stop iotedge
    ```

## <a name="chapter-11---create-table-service"></a><span data-ttu-id="dca2d-428">第 11-建立表格服務</span><span class="sxs-lookup"><span data-stu-id="dca2d-428">Chapter 11 - Create Table Service</span></span> 

<span data-ttu-id="dca2d-429">瀏覽回到 Azure 入口網站，您將在其中建立儲存體資源建立 Azure 資料表服務。</span><span class="sxs-lookup"><span data-stu-id="dca2d-429">Navigate back to your Azure Portal, where you will create an Azure Tables Service, by creating a Storage resource.</span></span>

1. <span data-ttu-id="dca2d-430">如果您尚未登入，登入[Azure 入口網站](https://portal.azure.com)。</span><span class="sxs-lookup"><span data-stu-id="dca2d-430">If not already signed in, log into the [Azure Portal](https://portal.azure.com).</span></span>

2. <span data-ttu-id="dca2d-431">登入之後，按一下**建立資源**，在左上角，並搜尋**儲存體帳戶**，然後按**Enter**金鑰，才能開始搜尋。</span><span class="sxs-lookup"><span data-stu-id="dca2d-431">Once logged in, click on **Create a resource**, in the top left corner, and search for **Storage account**, and press the **Enter** key, to start the search.</span></span>

3. <span data-ttu-id="dca2d-432">之後它會顯示，按一下 **儲存體帳戶-blob、 檔案、 資料表、 佇列**從清單中。</span><span class="sxs-lookup"><span data-stu-id="dca2d-432">Once it has appeared, click **Storage account - blob, file, table, queue** from the list.</span></span>

    ![搜尋儲存體帳戶](images/AzureLabs-Lab313-35.png)

4. <span data-ttu-id="dca2d-434">新的頁面將提供的描述**儲存體帳戶**服務。</span><span class="sxs-lookup"><span data-stu-id="dca2d-434">The new page will provide a description of the **Storage account** Service.</span></span> <span data-ttu-id="dca2d-435">在此提示的左下方，按一下**建立**按鈕，以建立這個執行個體服務。</span><span class="sxs-lookup"><span data-stu-id="dca2d-435">At the bottom left of this prompt, click the **Create** button, to create an instance of this Service.</span></span>

    ![建立儲存體執行個體](images/AzureLabs-Lab313-36.png)

5. <span data-ttu-id="dca2d-437">一旦您按下**建立**，面板會顯示：</span><span class="sxs-lookup"><span data-stu-id="dca2d-437">Once you have clicked on **Create**, a panel will appear:</span></span>

    1. <span data-ttu-id="dca2d-438">插入您想要**名稱**此服務執行個體 (*必須全部小寫*)。</span><span class="sxs-lookup"><span data-stu-id="dca2d-438">Insert your desired **Name** for this Service instance (*must be all lowercase*).</span></span>

    2. <span data-ttu-id="dca2d-439">針對**部署模型**，按一下**Resource manager**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-439">For **Deployment model**, click **Resource manager**.</span></span>

    3. <span data-ttu-id="dca2d-440">針對**帳戶種類**，使用下拉式功能表中，按一下**儲存體 (一般用途 v1)**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-440">For **Account kind**, using the dropdown menu, click **Storage (general purpose v1)**.</span></span>

    4. <span data-ttu-id="dca2d-441">按一下適當**位置**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-441">Click an appropriate **Location**.</span></span>
    
    5. <span data-ttu-id="dca2d-442">針對**複寫**下拉式功能表中，按一下**讀取-存取-異地備援儲存體 (RA-GRS)**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-442">For the **Replication** dropdown menu, click **Read-access-geo-redundant storage (RA-GRS)**.</span></span>

    6. <span data-ttu-id="dca2d-443">針對**效能**，按一下**標準**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-443">For **Performance**, click **Standard**.</span></span>

    7. <span data-ttu-id="dca2d-444">內**需要安全傳輸**區段中，按一下**停用**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-444">Within the **Secure transfer required** section, click **Disabled**.</span></span>

    8. <span data-ttu-id="dca2d-445">從**訂用帳戶**下拉式功能表中，按一下適當的訂用帳戶。</span><span class="sxs-lookup"><span data-stu-id="dca2d-445">From the **Subscription** dropdown menu, click an appropriate subscription.</span></span>

    9. <span data-ttu-id="dca2d-446">選擇**資源群組**或建立新的帳戶。</span><span class="sxs-lookup"><span data-stu-id="dca2d-446">Choose a **Resource Group** or create a new one.</span></span> <span data-ttu-id="dca2d-447">資源群組可用來監視、 控制存取權，佈建及管理的 Azure 資產的集合計費。</span><span class="sxs-lookup"><span data-stu-id="dca2d-447">A resource group provides a way to monitor, control access, provision, and manage, billing for a collection of Azure assets.</span></span> <span data-ttu-id="dca2d-448">建議將所有 Azure 服務在一般的資源群組相關聯 （例如例如這些課程中） 的單一專案保留）。</span><span class="sxs-lookup"><span data-stu-id="dca2d-448">It is recommended to keep all the Azure Services associated with a single project (e.g. such as these courses) under a common resource group).</span></span>

        > <span data-ttu-id="dca2d-449">如果您想要深入了解 Azure 資源群組，請遵循此[如何管理資源群組的連結](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-portal)。</span><span class="sxs-lookup"><span data-stu-id="dca2d-449">If you wish to read more about Azure Resource Groups, please follow this [link on how to manage a Resource Group](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-portal).</span></span>

    10. <span data-ttu-id="dca2d-450">離開**虛擬網路**作為**停用**，如果這是您的選項。</span><span class="sxs-lookup"><span data-stu-id="dca2d-450">Leave **Virtual networks** as **Disabled**, if this is an option for you.</span></span>

    11. <span data-ttu-id="dca2d-451">按一下 [建立] 。</span><span class="sxs-lookup"><span data-stu-id="dca2d-451">Click **Create**.</span></span>

        ![填入儲存體詳細資料](images/AzureLabs-Lab313-37.png)

6. <span data-ttu-id="dca2d-453">一旦您按下**建立**，您必須建立服務，這可能需要一分鐘。</span><span class="sxs-lookup"><span data-stu-id="dca2d-453">Once you have clicked on **Create**, you will have to wait for the Service to be created, this might take a minute.</span></span>

7. <span data-ttu-id="dca2d-454">通知會出現在入口網站中，一旦建立服務執行個體。</span><span class="sxs-lookup"><span data-stu-id="dca2d-454">A notification will appear in the Portal once the Service instance is created.</span></span> <span data-ttu-id="dca2d-455">按一下通知，以探索新的服務執行個體。</span><span class="sxs-lookup"><span data-stu-id="dca2d-455">Click on the notifications to explore your new Service instance.</span></span>

    ![新的儲存體通知](images/AzureLabs-Lab313-38.png)

8. <span data-ttu-id="dca2d-457">按一下 [**移至資源**按鈕，在該通知上方，而且您將會進入您新的儲存體服務執行個體概觀] 頁面。</span><span class="sxs-lookup"><span data-stu-id="dca2d-457">Click the **Go to resource** button in the notification, and you will be taken to your new Storage Service instance overview page.</span></span>

    ![前往資源](images/AzureLabs-Lab313-39.png)

9. <span data-ttu-id="dca2d-459">從 [概觀] 頁面的右手邊的側邊，按一下**資料表**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-459">From the overview page, to the right-hand side, click **Tables**.</span></span>
    
    ![資料表](images/AzureLabs-Lab313-40.png)

10. <span data-ttu-id="dca2d-461">在右側面板會變更以顯示**表格服務**資訊，您要在其中加入新的資料表。</span><span class="sxs-lookup"><span data-stu-id="dca2d-461">The panel on the right will change to show the **Table Service** information, wherein you need to add a new table.</span></span> <span data-ttu-id="dca2d-462">依序按一下 **+ 資料表**左上角的按鈕。</span><span class="sxs-lookup"><span data-stu-id="dca2d-462">Do this by clicking the **+ Table** button to the top-left corner.</span></span>

    ![開啟資料表](images/AzureLabs-Lab313-41.png)

11. <span data-ttu-id="dca2d-464">新的頁面將會顯示，您要在其中輸入**資料表名稱**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-464">A new page will be shown, wherein you need to enter a **Table name**.</span></span> <span data-ttu-id="dca2d-465">這是您用來在您的應用程式，在後續的章節 （建立函數應用程式和 Power BI） 中的資料所參考的名稱。</span><span class="sxs-lookup"><span data-stu-id="dca2d-465">This is the name you will use to refer to the data in your application in later Chapters (creating Function App, and Power BI).</span></span> <span data-ttu-id="dca2d-466">插入**IoTMessages**做為名稱 （您也可以選擇自己的圖片，只要記住它稍後在本文件中使用時），按一下 **確定**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-466">Insert **IoTMessages** as the name (you can choose your own, just remember it when used later in this document) and click **OK**.</span></span> 

12. <span data-ttu-id="dca2d-467">一旦建立新的資料表，您將能夠看到它內**表格服務**頁面 （位於底部）。</span><span class="sxs-lookup"><span data-stu-id="dca2d-467">Once the new table has been created, you will be able to see it within the **Table Service** page (at the bottom).</span></span>

    ![建立新資料表](images/AzureLabs-Lab313-42.png)  

13. <span data-ttu-id="dca2d-469">現在，按一下**存取金鑰**並採取一份**儲存體帳戶名稱**並**金鑰**（使用您在 [記事本]），您將使用這些值稍後在本課程中，建立時**Azure 函數應用程式**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-469">Now click on **Access keys** and take a copy of the **Storage account name** and **Key** (using your Notepad), you will use these values later in this course, when creating the **Azure Function App**.</span></span>

    ![建立新資料表](images/AzureLabs-Lab313-43.png) 

14. <span data-ttu-id="dca2d-471">同樣地，使用左邊的面板捲動到 *表格服務*區段，然後按一下**資料表**(或**瀏覽資料表**，在較新的入口網站中)，並採取一份**資料表 URL** （使用您在 記事本）。</span><span class="sxs-lookup"><span data-stu-id="dca2d-471">Using the panel on the left again, scroll to the *Table Service* section, and click **Tables** (or **Browse Tables**, in newer Portals) and take a copy of the **Table URL** (using your Notepad).</span></span> <span data-ttu-id="dca2d-472">連結資料表時，您會稍後在本課程中，使用此值您**Power BI**應用程式。</span><span class="sxs-lookup"><span data-stu-id="dca2d-472">You will use this value later in this course, when linking your table to your **Power BI** application.</span></span>

    ![建立新資料表](images/AzureLabs-Lab313-44.png)

## <a name="chapter-12---completing-the-azure-table"></a><span data-ttu-id="dca2d-474">第 12 章-完成 Azure 資料表</span><span class="sxs-lookup"><span data-stu-id="dca2d-474">Chapter 12 - Completing the Azure Table</span></span>

<span data-ttu-id="dca2d-475">既然您**表格服務**儲存體帳戶是否已設定，就可以開始將資料加入至它，將會用來儲存和擷取資訊。</span><span class="sxs-lookup"><span data-stu-id="dca2d-475">Now that your **Table Service** storage account has been setup, it is time to add data to it, which will be used to store and retrieve information.</span></span> <span data-ttu-id="dca2d-476">編輯您的資料表可透過**Visual Studio**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-476">The editing of your Tables can be done through **Visual Studio**.</span></span>

1. <span data-ttu-id="dca2d-477">開啟**Visual Studio** (**不**Visual Studio Code)。</span><span class="sxs-lookup"><span data-stu-id="dca2d-477">Open **Visual Studio** (**not** Visual Studio Code).</span></span>

2. <span data-ttu-id="dca2d-478">從功能表中，按一下**檢視 > Cloud Explorer**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-478">From the menu, click **View > Cloud Explorer**.</span></span>

    ![開啟 cloud explorer](images/AzureLabs-Lab313-45.png)

3. <span data-ttu-id="dca2d-480">**Cloud Explorer**會開啟為停駐項目 （耐心等候，因為載入可能會花費的時間）。</span><span class="sxs-lookup"><span data-stu-id="dca2d-480">The **Cloud Explorer** will open as a docked item (be patient, as loading may take time).</span></span>

    > [!WARNING] 
    > <span data-ttu-id="dca2d-481">如果您用來建立訂用帳戶您*儲存體帳戶*不可見，請確定您已：</span><span class="sxs-lookup"><span data-stu-id="dca2d-481">If the subscription you used to create your *Storage Accounts* is not visible, ensure that you have:</span></span> 
    > - <span data-ttu-id="dca2d-482">與您在 Azure 入口網站使用相同的帳戶登入。</span><span class="sxs-lookup"><span data-stu-id="dca2d-482">Logged in to the same account as the one you used for the Azure Portal.</span></span>
    > - <span data-ttu-id="dca2d-483">從 （您可能需要從您的帳戶設定套用篩選） 的 [帳戶管理] 頁面中選取您的訂用帳戶：</span><span class="sxs-lookup"><span data-stu-id="dca2d-483">Selected your subscription from the Account Management page (you may need to apply a filter from your account settings):</span></span>  
    >
    >   ![尋找訂用帳戶](images/AzureLabs-Lab313-46.png)

4. <span data-ttu-id="dca2d-485">Azure 雲端服務將會顯示。</span><span class="sxs-lookup"><span data-stu-id="dca2d-485">Your Azure cloud Services will be shown.</span></span> <span data-ttu-id="dca2d-486">尋找**儲存體帳戶**按一下箭號左側，展開您的帳戶。</span><span class="sxs-lookup"><span data-stu-id="dca2d-486">Find **Storage Accounts** and click the arrow to the left of that to expand your accounts.</span></span>

    ![開啟儲存體帳戶](images/AzureLabs-Lab313-47.png)

5. <span data-ttu-id="dca2d-488">一旦展開時，您新建立**儲存體帳戶**應該可用。</span><span class="sxs-lookup"><span data-stu-id="dca2d-488">Once expanded, your newly created **Storage account** should be available.</span></span> <span data-ttu-id="dca2d-489">按一下您的儲存體中，左邊的箭號之後，會展開，然後尋找**資料表**，按一下，以顯示旁的箭號**資料表**在最後一章中所建立。</span><span class="sxs-lookup"><span data-stu-id="dca2d-489">Click the arrow to the left of your storage, and then once that is expanded, find **Tables** and click the arrow next to that, to reveal the **Table** you created in the last Chapter.</span></span> <span data-ttu-id="dca2d-490">按兩下您**資料表**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-490">Double-click your **Table**.</span></span>

6. <span data-ttu-id="dca2d-491">您的資料表將會開啟您的 Visual Studio 視窗中央。</span><span class="sxs-lookup"><span data-stu-id="dca2d-491">Your table will be opened in the center of your Visual Studio window.</span></span> <span data-ttu-id="dca2d-492">按一下 [資料表] 圖示，以**+** （加上） 在其上。</span><span class="sxs-lookup"><span data-stu-id="dca2d-492">Click the table icon with the **+** (plus) on it.</span></span>

    ![加入新的資料表](images/AzureLabs-Lab313-48.png)

7. <span data-ttu-id="dca2d-494">接著會出現視窗提示要*新增實體*。</span><span class="sxs-lookup"><span data-stu-id="dca2d-494">A window will appear prompting for you to *Add Entity*.</span></span> <span data-ttu-id="dca2d-495">雖然會有三個屬性，您將建立一個的實體。</span><span class="sxs-lookup"><span data-stu-id="dca2d-495">You will create only one entity, though it will have three properties.</span></span> <span data-ttu-id="dca2d-496">您將會發現*PartitionKey*並*RowKey*已提供，這些資料表所用來尋找您的資料。</span><span class="sxs-lookup"><span data-stu-id="dca2d-496">You will notice that *PartitionKey* and *RowKey* are already provided, as these are used by the table to find your data.</span></span> 

    ![資料分割和資料列索引鍵](images/AzureLabs-Lab313-49.png)

8. <span data-ttu-id="dca2d-498">更新下列值：</span><span class="sxs-lookup"><span data-stu-id="dca2d-498">Update the following values:</span></span>

    - <span data-ttu-id="dca2d-499">名稱：**PartitionKey**，值：**PK_IoTMessages**</span><span class="sxs-lookup"><span data-stu-id="dca2d-499">Name: **PartitionKey**, Value: **PK_IoTMessages**</span></span> 

    - <span data-ttu-id="dca2d-500">名稱：**RowKey**，值：**RK_1_IoTMessages**</span><span class="sxs-lookup"><span data-stu-id="dca2d-500">Name: **RowKey**, Value: **RK_1_IoTMessages**</span></span> 

9. <span data-ttu-id="dca2d-501">然後，按一下 **將屬性加入**(至左下方的*加入實體*視窗)，並新增下列屬性：</span><span class="sxs-lookup"><span data-stu-id="dca2d-501">Then, click **Add property** (to the lower left of the *Add Entity* window) and add the following property:</span></span>

    - <span data-ttu-id="dca2d-502">**MessageContent**，作為*字串*，將值保留為空白。</span><span class="sxs-lookup"><span data-stu-id="dca2d-502">**MessageContent**, as a *string*, leave the Value empty.</span></span>

10. <span data-ttu-id="dca2d-503">您的資料表應符合下面的影像中的一個：</span><span class="sxs-lookup"><span data-stu-id="dca2d-503">Your table should match the one in the image below:</span></span>

    ![加入正確的值](images/AzureLabs-Lab313-50.png)

    > [!NOTE] 
    > <span data-ttu-id="dca2d-505">實體包含資料列索引鍵中的數字 1 的原因的原因是因為您可能想要新增更多的訊息，您想要實驗應該使用本課程。</span><span class="sxs-lookup"><span data-stu-id="dca2d-505">The reason why the entity has the number 1 in the row key, is because you might want to add more messages, should you desire to experiment further with this course.</span></span>

11. <span data-ttu-id="dca2d-506">按一下 **確定**完畢時。</span><span class="sxs-lookup"><span data-stu-id="dca2d-506">Click **OK** when you are finished.</span></span> <span data-ttu-id="dca2d-507">現在準備好可供您的資料表。</span><span class="sxs-lookup"><span data-stu-id="dca2d-507">Your table is now ready to be used.</span></span>

## <a name="chapter-13---create-an-azure-function-app"></a><span data-ttu-id="dca2d-508">第 13-建立 Azure 函式應用程式</span><span class="sxs-lookup"><span data-stu-id="dca2d-508">Chapter 13 - Create an Azure Function App</span></span> 

<span data-ttu-id="dca2d-509">就可以立即開始建立*Azure 函數應用程式*，則會藉由呼叫*IoT 中樞服務*儲存*IoT Edge*裝置中的訊息**資料表**服務，您在上一章中建立。</span><span class="sxs-lookup"><span data-stu-id="dca2d-509">It is now time to create an *Azure Function App*, which will be called by the *IoT Hub Service* to store the *IoT Edge* device messages in the **Table** Service, which you created in the previous Chapter.</span></span>

<span data-ttu-id="dca2d-510">首先，您必須建立一個檔案，可讓您的 Azure 函式，將您所需的程式庫。</span><span class="sxs-lookup"><span data-stu-id="dca2d-510">First, you need to create a file that will allow your Azure Function to load the libraries you need.</span></span>

1.  <span data-ttu-id="dca2d-511">開啟**記事本**(按下*Windows 鍵*，和型別*記事本*)。</span><span class="sxs-lookup"><span data-stu-id="dca2d-511">Open **Notepad** (press the *Windows Key*, and type *notepad*).</span></span>

    ![開啟 [記事本]](images/AzureLabs-Lab313-51.png)

2.  <span data-ttu-id="dca2d-513">使用 「 記事本 」 開啟，在其中插入以下的 JSON 結構。</span><span class="sxs-lookup"><span data-stu-id="dca2d-513">With Notepad open, insert the JSON structure below into it.</span></span> <span data-ttu-id="dca2d-514">完成後，會將它儲存為您的桌面上**project.json**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-514">Once you have done that, save it on your desktop as **project.json**.</span></span> <span data-ttu-id="dca2d-515">此檔案會定義您的函式會使用的程式庫。</span><span class="sxs-lookup"><span data-stu-id="dca2d-515">This file defines the libraries your function will use.</span></span> <span data-ttu-id="dca2d-516">如果您已使用 NuGet，它看起來會類似。</span><span class="sxs-lookup"><span data-stu-id="dca2d-516">If you have used NuGet, it will look familiar.</span></span>
    
    > [!WARNING]
    > <span data-ttu-id="dca2d-517">請務必確認名稱正確;請確定它並未**沒有.txt**副檔名。</span><span class="sxs-lookup"><span data-stu-id="dca2d-517">It is important that the naming is correct; ensure it does **NOT have a .txt** file extension.</span></span> <span data-ttu-id="dca2d-518">如需參考，請參閱以下內容：</span><span class="sxs-lookup"><span data-stu-id="dca2d-518">See below for reference:</span></span>
    >
...
3.  <span data-ttu-id="dca2d-520">登入[Azure 入口網站](https://portal.azure.com)。</span><span class="sxs-lookup"><span data-stu-id="dca2d-520">Log in to the [Azure Portal](https://portal.azure.com).</span></span>

4.  <span data-ttu-id="dca2d-521">一旦您登入，按一下**建立資源**在左上角，，然後搜尋**函式應用程式**，然後按**Enter**来搜尋的索引鍵。</span><span class="sxs-lookup"><span data-stu-id="dca2d-521">Once you are logged in, click on **Create a resource** in the top left corner, and search for **Function App**, and press the **Enter** key, to search.</span></span> <span data-ttu-id="dca2d-522">按一下 *函式應用程式*從結果中，若要開啟新的面板。</span><span class="sxs-lookup"><span data-stu-id="dca2d-522">Click *Function App* from the results, to open a new panel.</span></span>

    ![搜尋函式應用程式](images/AzureLabs-Lab313-53.png)

5.  <span data-ttu-id="dca2d-524">新的面板會提供的描述**函式應用程式**服務。</span><span class="sxs-lookup"><span data-stu-id="dca2d-524">The new panel will provide a description of the **Function App** Service.</span></span> <span data-ttu-id="dca2d-525">在此面板的左下方，按一下**建立**按鈕，以建立與這個關聯服務。</span><span class="sxs-lookup"><span data-stu-id="dca2d-525">At the bottom left of this panel, click the **Create** button, to create an association with this Service.</span></span>

    ![函式應用程式執行個體](images/AzureLabs-Lab313-54.png)

6.  <span data-ttu-id="dca2d-527">一旦您按下**建立**，填入下列：</span><span class="sxs-lookup"><span data-stu-id="dca2d-527">Once you have clicked on **Create**, fill in the following:</span></span>

    1. <span data-ttu-id="dca2d-528">針對**應用程式名稱**，插入您想要的名稱，此服務執行個體。</span><span class="sxs-lookup"><span data-stu-id="dca2d-528">For **App name**, insert your desired name for this Service instance.</span></span>

    2. <span data-ttu-id="dca2d-529">選取 **訂用帳戶**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-529">Select a **Subscription**.</span></span>

    3. <span data-ttu-id="dca2d-530">選取定價層適合您，如果這是第一次建立**函式應用程式服務**，免費層應該是您可以使用。</span><span class="sxs-lookup"><span data-stu-id="dca2d-530">Select the pricing tier appropriate for you, if this is the first time creating a **Function App Service**, a free tier should be available to you.</span></span>

    4. <span data-ttu-id="dca2d-531">選擇**資源群組**或建立新的帳戶。</span><span class="sxs-lookup"><span data-stu-id="dca2d-531">Choose a **Resource Group** or create a new one.</span></span> <span data-ttu-id="dca2d-532">資源群組可用來監視、 控制存取權，佈建及管理的 Azure 資產的集合計費。</span><span class="sxs-lookup"><span data-stu-id="dca2d-532">A resource group provides a way to monitor, control access, provision, and manage, billing for a collection of Azure assets.</span></span> <span data-ttu-id="dca2d-533">建議將所有 Azure 服務在一般的資源群組相關聯 （例如例如這些課程中） 的單一專案保留）。</span><span class="sxs-lookup"><span data-stu-id="dca2d-533">It is recommended to keep all the Azure Services associated with a single project (e.g. such as these courses) under a common resource group).</span></span>

        > <span data-ttu-id="dca2d-534">如果您想要深入了解 Azure 資源群組，請遵循此[如何管理資源群組的連結](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-portal)。</span><span class="sxs-lookup"><span data-stu-id="dca2d-534">If you wish to read more about Azure Resource Groups, please follow this [link on how to manage a Resource Group](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-portal).</span></span>

    5. <span data-ttu-id="dca2d-535">針對**OS**，按一下 Windows，因為這是預期的平台。</span><span class="sxs-lookup"><span data-stu-id="dca2d-535">For **OS**, click Windows, as that is the intended platform.</span></span>

    6. <span data-ttu-id="dca2d-536">選取 **主控方案**(使用本教學課程**取用方案**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-536">Select a **Hosting Plan** (this tutorial is using a **Consumption Plan**.</span></span>

    7. <span data-ttu-id="dca2d-537">選取 **位置**（上一個步驟中選擇您已建立的儲存體相同的位置）</span><span class="sxs-lookup"><span data-stu-id="dca2d-537">Select a **Location** (choose the same location as the storage you have built in the previous step)</span></span>

    8. <span data-ttu-id="dca2d-538">針對**儲存體**一節**您必須選取您在上一個步驟中建立的儲存體服務**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-538">For the **Storage** section, **you must select the Storage Service you created in the previous step**.</span></span>

    9. <span data-ttu-id="dca2d-539">您不需要*Application Insights*在此應用程式，因此您將它保留**關閉**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-539">You will not need *Application Insights* in this app, so feel free to leave it **Off**.</span></span>

    10. <span data-ttu-id="dca2d-540">按一下 [建立] 。</span><span class="sxs-lookup"><span data-stu-id="dca2d-540">Click **Create**.</span></span>

        ![建立新的執行個體](images/AzureLabs-Lab313-55.png)

7.  <span data-ttu-id="dca2d-542">一旦您按下**建立**，您必須建立服務，這可能需要一分鐘。</span><span class="sxs-lookup"><span data-stu-id="dca2d-542">Once you have clicked on **Create**, you will have to wait for the Service to be created, this might take a minute.</span></span>

8.  <span data-ttu-id="dca2d-543">通知會出現在入口網站中，一旦建立服務執行個體。</span><span class="sxs-lookup"><span data-stu-id="dca2d-543">A notification will appear in the Portal once the Service instance is created.</span></span>

    ![新的通知](images/AzureLabs-Lab313-56.png)

9.  <span data-ttu-id="dca2d-545">部署成功之後，按一下 通知 （完成）。</span><span class="sxs-lookup"><span data-stu-id="dca2d-545">Click on the notification, once deployment is successful (has finished).</span></span>

10. <span data-ttu-id="dca2d-546">按一下 **移至資源**通知，以探索新的服務執行個體中的按鈕。</span><span class="sxs-lookup"><span data-stu-id="dca2d-546">Click the **Go to resource** button in the notification to explore your new Service instance.</span></span> 

    ![前往資源](images/AzureLabs-Lab313-57.png)

11. <span data-ttu-id="dca2d-548">在新的面板的左側，按一下**+** （加號） 旁的圖示*函式*，以建立新的函式。</span><span class="sxs-lookup"><span data-stu-id="dca2d-548">In the left side of the new panel, click the **+** (plus) icon next to *Functions*, to create a new function.</span></span>

    ![新增新的函式](images/AzureLabs-Lab313-58.png)

12. <span data-ttu-id="dca2d-550">在中央窗格中，**函式**建立視窗會出現。</span><span class="sxs-lookup"><span data-stu-id="dca2d-550">Within the central panel, the **Function** creation window will appear.</span></span> <span data-ttu-id="dca2d-551">此外，向下捲動，然後按一下**自訂函式**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-551">Scroll down further, and click on **Custom function**.</span></span>

    ![自訂函式](images/AzureLabs-Lab313-59.png)

13. <span data-ttu-id="dca2d-553">下一步 頁面上，直到您找到捲動**IoT 中樞 （事件中樞）**，然後按一下它。</span><span class="sxs-lookup"><span data-stu-id="dca2d-553">Scroll down the next page, until you find **IoT Hub (Event Hub)**, then click on it.</span></span>

    ![自訂函式](images/AzureLabs-Lab313-60.png)

14. <span data-ttu-id="dca2d-555">在  **IoT 中樞 （事件中樞）** 刀鋒視窗中，將**語言**來**C#** ，然後按一下**新**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-555">In the **IoT Hub (Event Hub)** blade, set the **Language** to **C#** and then click on **new**.</span></span>

    ![自訂函式](images/AzureLabs-Lab313-61.png)

15. <span data-ttu-id="dca2d-557">在視窗中會出現，請確定**IoT 中樞**已選取和名稱*IoT 中樞*欄位對應的名稱取代您*IoT 中樞服務*您具有先前所建立 ([在步驟 8，第 3 章](#chapter-3---the-iot-hub-service))。</span><span class="sxs-lookup"><span data-stu-id="dca2d-557">In the window that will appear, make sure that **IoT Hub** is selected and the name of the *IoT Hub* field corresponds with the name of your *IoT Hub Service* that you have created previously ([in step 8, of Chapter 3](#chapter-3---the-iot-hub-service)).</span></span> <span data-ttu-id="dca2d-558">然後按一下**選取** 按鈕。</span><span class="sxs-lookup"><span data-stu-id="dca2d-558">Then click the **Select** button.</span></span>

    ![自訂函式](images/AzureLabs-Lab313-62.png)

16. <span data-ttu-id="dca2d-560">回到**IoT 中樞 （事件中樞）** 刀鋒視窗中，按一下**建立**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-560">Back on the **IoT Hub (Event Hub)** blade, click on **Create**.</span></span>

    ![自訂函式](images/AzureLabs-Lab313-63.png)

17. <span data-ttu-id="dca2d-562">您將會重新導向至函式編輯器。</span><span class="sxs-lookup"><span data-stu-id="dca2d-562">You will be redirected to the function editor.</span></span>

    ![自訂函式](images/AzureLabs-Lab313-64.png)

18. <span data-ttu-id="dca2d-564">刪除所有程式碼，並將它取代為下列：</span><span class="sxs-lookup"><span data-stu-id="dca2d-564">Delete all the code in it and replace it with the following:</span></span>
...
19. <span data-ttu-id="dca2d-565">變更下列變數，讓它們對應至適當的值 (**表格**並**儲存體**的值，從[步驟 11 日及 13 日分別第 11 章](#chapter-11---create-table-service))，您會在中找到您**儲存體帳戶**:</span><span class="sxs-lookup"><span data-stu-id="dca2d-565">Change the following variables, so that they correspond to the appropriate values (**Table** and **Storage** values, from [step 11 and 13, respectively, of Chapter 11](#chapter-11---create-table-service)), that you will find in your **Storage Account**:</span></span>

    - <span data-ttu-id="dca2d-566">**tableName**，名稱與您**表格**位於您**儲存體帳戶**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-566">**tableName**, with the name of your **Table** located in your **Storage Account**.</span></span>
    - <span data-ttu-id="dca2d-567">**tableURL**，使用的 URL 您**表格**位於您**儲存體帳戶**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-567">**tableURL**, with the URL of your **Table** located in your **Storage Account**.</span></span>
    - <span data-ttu-id="dca2d-568">**storageAccountName**，對應的名稱值的名稱與您**儲存體帳戶**名稱。</span><span class="sxs-lookup"><span data-stu-id="dca2d-568">**storageAccountName**, with the name of the value corresponding with the name of your **Storage Account** name.</span></span>
    - <span data-ttu-id="dca2d-569">**storageAccountKey**，您已取得您先前建立的儲存體服務中的金鑰。</span><span class="sxs-lookup"><span data-stu-id="dca2d-569">**storageAccountKey**, with the Key you have obtained in the Storage Service you have created previously.</span></span>

    ![自訂函式](images/AzureLabs-Lab313-65.png)

20. <span data-ttu-id="dca2d-571">中位置的程式碼，請按一下**儲存**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-571">With the code in place, click **Save**.</span></span>

21. <span data-ttu-id="dca2d-572">接下來，按一下**\<** （箭頭） 圖示，在頁面的右手邊。</span><span class="sxs-lookup"><span data-stu-id="dca2d-572">Next, click the **\<** (arrow) icon, on the right-hand side of the page.</span></span>

    ![自訂函式](images/AzureLabs-Lab313-66.png)

22. <span data-ttu-id="dca2d-574">從右邊的窗格中將投影片。</span><span class="sxs-lookup"><span data-stu-id="dca2d-574">A panel will slide in from the right.</span></span> <span data-ttu-id="dca2d-575">在該面板中，按一下**上傳**，以及*檔案瀏覽器*會出現。</span><span class="sxs-lookup"><span data-stu-id="dca2d-575">In that panel, click **Upload**, and a *File Browser* will appear.</span></span>

23. <span data-ttu-id="dca2d-576">瀏覽至，然後按一下 []， **project.json**中建立的檔案**記事本**之前，然後按一下 [**開啟**] 按鈕。</span><span class="sxs-lookup"><span data-stu-id="dca2d-576">Navigate to, and click, the **project.json** file, which you created in **Notepad** previously, and then click the **Open** button.</span></span> <span data-ttu-id="dca2d-577">此檔案會定義您的函式會使用的程式庫。</span><span class="sxs-lookup"><span data-stu-id="dca2d-577">This file defines the libraries that your function will use.</span></span>

    ![自訂函式](images/AzureLabs-Lab313-67.png)

24. <span data-ttu-id="dca2d-579">檔案已上傳時，它會出現在右側面板。</span><span class="sxs-lookup"><span data-stu-id="dca2d-579">When the file has uploaded, it will appear in the panel on the right.</span></span> <span data-ttu-id="dca2d-580">按一下將內開啟它**函式**編輯器。</span><span class="sxs-lookup"><span data-stu-id="dca2d-580">Clicking it will open it within the **Function** editor.</span></span> <span data-ttu-id="dca2d-581">它必須看起來**完全**與下圖相同。</span><span class="sxs-lookup"><span data-stu-id="dca2d-581">It must look **exactly** the same as the next image.</span></span>

    ![自訂函式](images/AzureLabs-Lab313-68.png)

25. <span data-ttu-id="dca2d-583">此時就很好測試您的函式的功能，將訊息儲存在您*資料表*。</span><span class="sxs-lookup"><span data-stu-id="dca2d-583">At this point it would be good to test the capability of your Function to store the message on your *Table*.</span></span> <span data-ttu-id="dca2d-584">在右上方的 [] 視窗中，按一下**測試**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-584">On the top right side of the window, click on **Test**.</span></span>

    ![自訂函式](images/AzureLabs-Lab313-69.png)

26. <span data-ttu-id="dca2d-586">將訊息插入上**要求本文**，如上顯示的影像上方，然後按一下**執行**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-586">Insert a message on the **Request body**, as shown in the image above, and click on **Run**.</span></span> 

27. <span data-ttu-id="dca2d-587">此函式會執行，顯示的結果狀態 (您會注意到綠色**狀態 202 已接受**上面*輸出*視窗中，這表示它已成功的呼叫):</span><span class="sxs-lookup"><span data-stu-id="dca2d-587">The function will run, displaying the result status (you will notice the green **Status 202 Accepted**, above the *Output* window, which means it was a successful call):</span></span>

    ![輸出結果](images/AzureLabs-Lab313-70.png)

## <a name="chapter-14---view-active-messages"></a><span data-ttu-id="dca2d-589">第 14 章-檢視作用中的訊息</span><span class="sxs-lookup"><span data-stu-id="dca2d-589">Chapter 14 - View active messages</span></span>

<span data-ttu-id="dca2d-590">如果您現在可以開啟 Visual Studio (**未**Visual Studio Code)，您可以將視覺化您測試訊息的結果，因為它會儲存在*MessageContent*區域的字串。</span><span class="sxs-lookup"><span data-stu-id="dca2d-590">If you now open Visual Studio (**not** Visual Studio Code), you can visualize your test message result, as it will be stored in the *MessageContent* string area.</span></span>

![自訂函式](images/AzureLabs-Lab313-71.png)

<span data-ttu-id="dca2d-592">表格服務和函式應用程式就地，Ubuntu 裝置上的訊息會出現在您*IoTMessages*資料表。</span><span class="sxs-lookup"><span data-stu-id="dca2d-592">With the Table Service and Function App in place, your Ubuntu device messages will appear in your *IoTMessages* Table.</span></span> <span data-ttu-id="dca2d-593">如果尚未執行，同樣地，啟動您的裝置，您將能夠看到結果訊息，從您的裝置和您的資料表內的模組，使用 Visual Studio *Cloud Explorer*。</span><span class="sxs-lookup"><span data-stu-id="dca2d-593">If not already running, start your device again, and you will be able to see the result messages from your device, and module, within your Table, through using Visual Studio *Cloud Explorer*.</span></span>

![將資料視覺化](images/AzureLabs-Lab313-72.png)


## <a name="chapter-15---power-bi-setup"></a><span data-ttu-id="dca2d-595">第 15 章-Power BI 設定</span><span class="sxs-lookup"><span data-stu-id="dca2d-595">Chapter 15 - Power BI Setup</span></span>

<span data-ttu-id="dca2d-596">若要從您的 IOT 裝置會設定將資料視覺化**Power BI** （桌面版），要從中收集資料*資料表*您剛才建立的服務。</span><span class="sxs-lookup"><span data-stu-id="dca2d-596">To visualize the data from your IOT device you will setup **Power BI** (desktop version), to collect the data from the *Table* Service, which you just created.</span></span> <span data-ttu-id="dca2d-597">*HoloLens*版本的 Power BI 會接著使用該資料以視覺化方式檢視結果。</span><span class="sxs-lookup"><span data-stu-id="dca2d-597">The *HoloLens* version of Power BI will then use that data to visualize the result.</span></span>

1.  <span data-ttu-id="dca2d-598">開啟 Windows 10 上的 Microsoft Store 並搜尋**Power BI Desktop**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-598">Open the Microsoft Store on Windows 10 and search for **Power BI Desktop**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-73.png)

2.  <span data-ttu-id="dca2d-600">下載應用程式。</span><span class="sxs-lookup"><span data-stu-id="dca2d-600">Download the application.</span></span> <span data-ttu-id="dca2d-601">一旦下載完成，請開啟它。</span><span class="sxs-lookup"><span data-stu-id="dca2d-601">Once it has finished downloading, open it.</span></span>

3.  <span data-ttu-id="dca2d-602">登入*Power BI*與您**Microsoft 365 帳戶**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-602">Log into *Power BI* with your **Microsoft 365 account**.</span></span> <span data-ttu-id="dca2d-603">您可能會重新導向至瀏覽器中，登入。</span><span class="sxs-lookup"><span data-stu-id="dca2d-603">You may be redirected to a browser, to sign up.</span></span> <span data-ttu-id="dca2d-604">一旦您已註冊，返回 Power BI 應用程式，並再次登入。</span><span class="sxs-lookup"><span data-stu-id="dca2d-604">Once you are signed up, go back to the Power BI app, and sign in again.</span></span>

4.  <span data-ttu-id="dca2d-605">按一下 **取得資料**，然後按一下**更多...**.</span><span class="sxs-lookup"><span data-stu-id="dca2d-605">Click on **Get Data** and then click on **More...**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-74.png)

5.  <span data-ttu-id="dca2d-607">按一下  **Azure**， **Azure 資料表儲存體**，然後按一下**Connect**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-607">Click **Azure**, **Azure Table Storage**, then click on **Connect**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-75.png)

6.  <span data-ttu-id="dca2d-609">系統會提示您插入**adresa URL Tabulky**稍早收集到的 ([在步驟 13 小時，共 11 章](#chapter-11---create-table-service))，建立您的資料表服務時。</span><span class="sxs-lookup"><span data-stu-id="dca2d-609">You will be prompted to insert the **Table URL** that you collected earlier ([in step 13 of Chapter 11](#chapter-11---create-table-service)), while creating your Table Service.</span></span> <span data-ttu-id="dca2d-610">在插入 URL 之後, 刪除路徑參考到資料表"子資料夾 」 （這是 IoTMessages，本課程中） 的一部分。</span><span class="sxs-lookup"><span data-stu-id="dca2d-610">After inserting the URL, delete the portion of the path referring to the Table "sub-folder" (which was IoTMessages, in this course).</span></span> <span data-ttu-id="dca2d-611">最後的結果應該如下圖所示。</span><span class="sxs-lookup"><span data-stu-id="dca2d-611">The final result should be as displayed in the image below.</span></span> <span data-ttu-id="dca2d-612">然後按一下**確定**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-612">Then click on **OK**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-76.png)

7.  <span data-ttu-id="dca2d-614">系統會提示您插入**儲存體金鑰**您記下 ([的步驟 11 第 11 章](#chapter-11---create-table-service)) 先前在建立您的資料表儲存體。</span><span class="sxs-lookup"><span data-stu-id="dca2d-614">You will be prompted to insert the **Storage Key** that you noted ([in step 11 of Chapter 11](#chapter-11---create-table-service)) earlier while creating your Table Storage.</span></span> <span data-ttu-id="dca2d-615">然後按一下**Connect**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-615">Then click on **Connect**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-77.png)  

8. <span data-ttu-id="dca2d-617">A**導覽器 面板**將會顯示，勾選您表旁邊的方塊，然後按一下**負載**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-617">A **Navigator Panel** will be displayed, tick the box next to your Table and click on **Load**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-78.png)  

9. <span data-ttu-id="dca2d-619">Power BI 中，現在已載入您的資料表，但需要的查詢，以在其中顯示的值。</span><span class="sxs-lookup"><span data-stu-id="dca2d-619">Your table has now been loaded on Power BI, but it requires a query to display the values in it.</span></span> <span data-ttu-id="dca2d-620">若要這樣做，請以滑鼠右鍵按一下資料表名稱，其位於**欄位面板**螢幕的右上方。</span><span class="sxs-lookup"><span data-stu-id="dca2d-620">To do so, right-click on the table name located in the **FIELDS panel** at the right side of the screen.</span></span> <span data-ttu-id="dca2d-621">然後按一下**編輯查詢**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-621">Then click on **Edit Query**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-79.png) 

10. <span data-ttu-id="dca2d-623">A **Power Query 編輯器**會開啟新視窗，顯示您的資料表。</span><span class="sxs-lookup"><span data-stu-id="dca2d-623">A **Power Query Editor**  will open up as a new window, displaying your table.</span></span> <span data-ttu-id="dca2d-624">按一下 上一字**記錄**內*內容*資料表資料行，以視覺化方式檢視您儲存的內容。</span><span class="sxs-lookup"><span data-stu-id="dca2d-624">Click on the word **Record** within the *Content* column of the table, to visualize your stored content.</span></span>

    ![Power BI](images/AzureLabs-Lab313-80.png)    

11. <span data-ttu-id="dca2d-626">按一下  **Into 資料表**，在視窗的左上方。</span><span class="sxs-lookup"><span data-stu-id="dca2d-626">Click on **Into Table**, at the top-left of the window.</span></span> 

    ![Power BI](images/AzureLabs-Lab313-81.png)

12. <span data-ttu-id="dca2d-628">按一下 **關閉並套用**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-628">Click on **Close & Apply**.</span></span>

    ![Power BI](images/AzureLabs-Lab313-82.png)

13. <span data-ttu-id="dca2d-630">在載入此查詢中，完成後**欄位面板**，在畫面的右側，勾選 參數對應的方塊**名稱**並**值**至以視覺化方式檢視**MessageContent**資料行內容。</span><span class="sxs-lookup"><span data-stu-id="dca2d-630">Once it has finished loading the query, within the **FIELDS panel**, on the right side of the screen, tick the boxes corresponding to the parameters **Name** and **Value**, to visualize the **MessageContent** column content.</span></span>

    ![Power BI](images/AzureLabs-Lab313-83.png)

14. <span data-ttu-id="dca2d-632">按一下 **藍色的磁碟片圖示**在左上方的 儲存您所選擇的資料夾中的工作 視窗。</span><span class="sxs-lookup"><span data-stu-id="dca2d-632">Click on the **blue disk icon** at the top left of the window to save your work in a folder of your choice.</span></span>

    ![Power BI](images/AzureLabs-Lab313-84.png)

15. <span data-ttu-id="dca2d-634">您現在可以按一下 [發行] 按鈕上, 傳至您的工作區的資料表。</span><span class="sxs-lookup"><span data-stu-id="dca2d-634">You can now click on the Publish button to upload your table to your Workspace.</span></span> <span data-ttu-id="dca2d-635">出現提示時，按一下**我的工作區**然後按一下*選取*。</span><span class="sxs-lookup"><span data-stu-id="dca2d-635">When prompted, click **My workspace** and click *Select*.</span></span> <span data-ttu-id="dca2d-636">等候它顯示在提交的成功的結果。</span><span class="sxs-lookup"><span data-stu-id="dca2d-636">Wait for it to display the successful result of the submission.</span></span>
...
> [!WARNING]
> <span data-ttu-id="dca2d-639">下一個章節是特定的 HoloLens。</span><span class="sxs-lookup"><span data-stu-id="dca2d-639">The following Chapter is HoloLens specific.</span></span> <span data-ttu-id="dca2d-640">Power BI 目前不提供為沈浸式應用程式，不過您可以在 Windows 混合實境入口網站 （也稱為 Cliff 房屋） 來執行桌面版本透過桌面應用程式。</span><span class="sxs-lookup"><span data-stu-id="dca2d-640">Power BI is not currently availble as an immersive application, however you can run the desktop version in the Windows Mixed Reality Portal (aka Cliff House), through the Desktop app.</span></span>

## <a name="chapter-16---display-power-bi-data-on-hololens"></a><span data-ttu-id="dca2d-641">第 16 章-HoloLens 上顯示 Power BI 資料</span><span class="sxs-lookup"><span data-stu-id="dca2d-641">Chapter 16 - Display Power BI data on HoloLens</span></span>

1. <span data-ttu-id="dca2d-642">在您的 HoloLens 上登入**Microsoft Store**，依序點選應用程式清單中的圖示。</span><span class="sxs-lookup"><span data-stu-id="dca2d-642">On your HoloLens, log in to the **Microsoft Store**, by tapping on its icon in the applications list.</span></span>

    ![Power BI HL](images/AzureLabs-Lab313-87.png)

2. <span data-ttu-id="dca2d-644">搜尋，然後下載**Power BI**應用程式。</span><span class="sxs-lookup"><span data-stu-id="dca2d-644">Search and then download the **Power BI** application.</span></span>

    ![Power BI HL](images/AzureLabs-Lab313-88.png)

3. <span data-ttu-id="dca2d-646">開始**Power BI**從您的應用程式清單。</span><span class="sxs-lookup"><span data-stu-id="dca2d-646">Start **Power BI** from your applications list.</span></span> 

4. <span data-ttu-id="dca2d-647">**Power BI**可能會要求您登入您**Microsoft 365 帳戶**。</span><span class="sxs-lookup"><span data-stu-id="dca2d-647">**Power BI** might ask you to login to your **Microsoft 365 account**.</span></span>

5. <span data-ttu-id="dca2d-648">一次，應用程式內的工作區應該預設會顯示在下圖所示。</span><span class="sxs-lookup"><span data-stu-id="dca2d-648">Once inside the app, the workspace should display by default as shown in the image below.</span></span> <span data-ttu-id="dca2d-649">如果，不會發生，只要按一下視窗左邊的工作區圖示。</span><span class="sxs-lookup"><span data-stu-id="dca2d-649">If that does not happen, simply click on the workspace icon on the left side of the window.</span></span>

    ![Power BI HL](images/AzureLabs-Lab313-89.png)

## <a name="your-finished-your-iot-hub-application"></a><span data-ttu-id="dca2d-651">您已完成您的 IoT 中樞應用程式</span><span class="sxs-lookup"><span data-stu-id="dca2d-651">Your finished your IoT Hub application</span></span>

<span data-ttu-id="dca2d-652">恭喜，您已成功建立 IoT 中樞服務，以模擬的虛擬機器的 Edge 裝置。</span><span class="sxs-lookup"><span data-stu-id="dca2d-652">Congratulations, you have successfully created an IoT Hub Service, with a simulated Virtual Machine Edge device.</span></span> <span data-ttu-id="dca2d-653">您的裝置可以進行通訊的機器學習模型到 Azure 資料表服務，透過 Azure 函式應用程式，這是讀取到 Power BI，並以視覺化方式檢視內 Microsoft HoloLens 的結果。</span><span class="sxs-lookup"><span data-stu-id="dca2d-653">Your device can  communicate the results of a machine learning model to an Azure Table Service, facilitated by an Azure Function App, which is read into Power BI, and visualized within a Microsoft HoloLens.</span></span>
 
![Power BI](images/AzureLabs-Lab313-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="dca2d-655">Bonus 練習</span><span class="sxs-lookup"><span data-stu-id="dca2d-655">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="dca2d-656">練習 1</span><span class="sxs-lookup"><span data-stu-id="dca2d-656">Exercise 1</span></span>

<span data-ttu-id="dca2d-657">展開 儲存在資料表中的訊息結構，則會顯示為圖形。</span><span class="sxs-lookup"><span data-stu-id="dca2d-657">Expand the messaging structure stored in the table and display it as a graph.</span></span> <span data-ttu-id="dca2d-658">您可能想要收集更多資料，並將它儲存在相同資料表中，稍後再顯示。</span><span class="sxs-lookup"><span data-stu-id="dca2d-658">You might want to collect more data and store it in the same table, to be later displayed.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="dca2d-659">練習 2</span><span class="sxs-lookup"><span data-stu-id="dca2d-659">Exercise 2</span></span>

<span data-ttu-id="dca2d-660">建立其他 IoT 面板，以便它可以透過分析攝影機擷取映像部署至 「 相機擷取 」 模組。</span><span class="sxs-lookup"><span data-stu-id="dca2d-660">Create an additional "camera capture" module to be deployed on the IoT board, so that it can capture images through the camera to be analyzed.</span></span>
