...
ms.locfileid: "86408196"
---
# <a name="porting-guides"></a>移植指南

## <a name="overview"></a>概觀

Windows 10 包含沉浸式和全像攝影耳機的直接支援。 如果您已針對其他裝置（例如 Oculus go Rift 或 HTC Vive）建立內容，這些專案會相依于存在於作業系統平臺 API 的程式庫。 將現有的 Win32 Unity VR 應用程式帶入 Windows Mixed Reality 牽涉到將廠商專屬的 VR Sdk 重定使用到 Unity 的跨廠商 VR Api。
...
3. **升級任何中介軟體、外掛程式或元件。** 如果您的應用程式包含任何元件，最好先升級至最新版本。
...
* 如果您使用 Visual Studio 然後升級至最新的組建
* 請參閱 Visual Studio 2019 底下[的 [安裝工具](install-the-tools.md#installation-checklist)] 頁面

### <a name="common-step-4-choose-the-correct-adapter"></a>一般步驟4：選擇正確的介面卡
...
### <a name="unity-step-1-review-the-common-porting-steps-listed-above"></a>Unity 步驟1：檢查上面列出的一般移植步驟

請檢查上面列出的常見步驟，以確定您的開發環境已正確設定。 在步驟 #3 中，如果您要使用 Visual Studio，您應該選取 [**使用 Unity 工作負載進行遊戲開發**]。 您可以取消選取 [Unity Editor optional] 元件，因為在下一個步驟中，您將會安裝較新版本的 Unity。
...
2. 在開始之前，請先儲存您的專案複本
3. 如果您的專案是以較舊版本的 Unity 建立，請參閱升級 Unity 中的可用[檔](https://docs.unity3d.com/Manual/UpgradeGuides.html)。
...
有了任何 Unity 更新，您很有可能需要更新您的遊戲或應用程式所依賴的一個或多個中介軟體套件。 此外，最新的中介軟體也會在移植程式的其餘部分中，增加成功的可能性。

### <a name="unity-step-4-target-your-application-to-run-on-win32"></a>Unity 步驟4：將您的應用程式設定為在 Win32 上執行
...
### <a name="unity-step-5-setup-your-windows-mixed-reality-hardware"></a>Unity 步驟5：設定您的 Windows Mixed Reality 硬體
...
### <a name="unity-step-6-target-your-application-to-run-on-windows-mixed-reality"></a>Unity 步驟6：將您的應用程式設為目標以在 Windows Mixed Reality 上執行
1. 首先，您必須移除或有條件地編譯特定 VR SDK 特定的任何其他程式庫支援。 這些資產通常會以與其他 VR Sdk 不相容的方式來變更專案的設定和屬性，例如 Windows Mixed Reality。
    * 例如，如果您的專案參考 SteamVR SDK，您必須將專案更新為，改為使用 Unity 的通用 VR Api，同時支援 Windows Mixed Reality 和 SteamVR。
...
### <a name="unity-step-7-use-the-stage-to-place-content-on-the-floor"></a>Unity 步驟7：使用階段將內容放置在樓層上
...
### <a name="unity-step-8-work-through-your-input-model"></a>Unity 步驟8：使用您的輸入模型
...
### <a name="unity-step-9-performance-testing-and-tuning"></a>Unity 步驟9：效能測試和微調
