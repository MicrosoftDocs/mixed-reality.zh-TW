...
[手勢](gestures.md)會將使用者意圖變成動作。 透過手勢, 使用者可以與全息影像互動。 在本課程中, 我們將學習如何追蹤使用者的手、回應使用者輸入, 並根據手中的狀態和位置提供意見反應給使用者。
...
* 下載專案所需的[檔案](https://github.com/Microsoft/HolographicAcademy/archive/Holograms-211-Gesture.zip)。 需要 Unity 2017.2 或更新版本。
...
3. 按一下 [階層] 面板中的 [ **InputManager** ], 然後將**CursorWithFeedback**物件從階層拖曳至 InputManager 的**SimpleSinglePointerSelector**的 [**游標**] 欄位 (位於底部)偵測**器**。
4. 按一下階層中的 [ **CursorWithFeedback** ]。
...
1. 在偵測器中按一下 [上一頁], 然後展開 [**檢查**] 中的 [**關鍵字**] 區段。
2. 按一下右手 **+** 邊的以加入新的關鍵字。
