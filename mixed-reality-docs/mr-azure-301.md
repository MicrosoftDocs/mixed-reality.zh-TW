...
> [!NOTE]
> 在此課程中，您將不需要有動作控制器。 如果您需要支援設定沉浸式耳機，請[遵循下列步驟](https://support.microsoft.com/help/4043101/windows-10-set-up-windows-mixed-reality)。
...
5.  然後在*Translator*類別內新增下列變數：
    ...
    > - 有[更多互動式頁面涵蓋可用的語言](https://www.microsoft.com/translator/business/languages/)，不過請注意，只有當網站語言設定為 ' ' （而 Microsoft 網站可能會重新導向至您的原生語言）時，頁面才會運作。 您可以變更頁面底部的網站語言，或變更 URL。
...
1.  回到 Unity 編輯器中，按一下 [**腳本**] 資料夾中的 [*結果* *] 類別，* 並將其拖曳至 [階層]*面板*中的**主要相機**物件。
...
- 當您執行場景時，您的麥克風會運作（如果不是，請檢查您連接的麥克風是否為*預設*裝置，而且您已在[Windows 中正確設定](https://support.microsoft.com/help/4027981/windows-how-to-set-up-and-test-microphones-in-windows-10)）。
