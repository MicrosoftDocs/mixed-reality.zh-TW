...
為了提供最佳的使用者體驗, 請務必瞭解會影響 HoloLens 所收集的空間對應資料品質的因素。
...
* **使用者動作**
   * 使用者在其環境中的移動方式會決定要掃描環境的情況, 因此使用者可能需要指引才能達成良好的掃描。
   * 用於掃描的相機會在70角度的錐形中提供資料, 從最少0.8 計量到最多從相機的3.1 計量距離。 真實世界的表面只會在此視圖的範圍內掃描。 請注意, 這些值在未來的版本中可能會變更。
   * 如果使用者永遠不會在物件的3.1 計量內, 則不會進行掃描。
   * 如果使用者只從小於0.8 計量的距離流覽物件, 則不會進行掃描 (這可避免掃描使用者的手)。
   * 如果使用者從未向上尋找 (這相當正常), 則可能不會掃描上限。
   * 如果使用者從未看過傢俱或牆後, 則不會掃描其所 pixels occluded 的物件。
   * 表面通常會在以較高的品質來觀看, 而不是以淺層角度進行掃描。
   * 如果 HoloLens 的頭追蹤系統暫時失敗 (可能是因為快速的使用者動作、光源不佳、featureless 牆或相機已涵蓋), 這可能會導致空間對應資料發生錯誤。 當使用者繼續四處移動並掃描其環境時, 將會在一段時間後更正任何這類錯誤。

* **Surface 材質**
   * 在真實世界表面上找到的材質會有很大的差異。 這些會影響空間對應資料的品質, 因為它們會影響紅外線光線的反映方式。
   * 深色表面可能不會掃描到靠近相機, 因為它們會反映較少的光線。
   * 有些表面可能會變暗, 以致于無法從任何距離掃描太少的光線, 因此它們會在介面的位置 (有時也會在介面背後) 引進洞錯誤。
   * 特別是有光澤的表面, 只有在看到「前端」時才會掃描, 而不是從淺層角度觀看。
   * 鏡像, 因為它們會建立 illusory 的實際空間和表面反射, 所以可能會造成洞錯誤和 hallucination 錯誤。

* **場景動作**
   * 空間對應可快速適應環境中的變更, 例如移動人員或開啟和關閉門。
   * 不過, 空間對應只有在用於掃描的相機清楚可見時, 才會調整區域中的變更。
   * 因此, 此調整可能會落後于現實, 這可能會造成洞或 hallucination 錯誤。
   * 例如, 使用者會掃描朋友, 然後在朋友離開房間時輪流流覽。 Friend 的「准刪除」標記法 (hallucination 錯誤) 將會保存在空間對應資料中, 直到使用者重新開啟, 然後重新掃描 friend 所在的空間為止。

* **光源干擾**
   * 場景中的環境紅外線燈可能會干擾掃描, 例如, 透過視窗的強大陽光。
   * 特別是光亮的表面可能會干擾附近的表面掃描, 而光線會導致偏差錯誤。
   * 將光源直接反映在相機中的發光表面可能會造成浮動的 hallucinations, 或藉由延遲調節至場景動作, 而干擾附近的空間。
   * 相同房間內的兩部 HoloLens 裝置不應該干擾另一部, 但如果有超過五個 HoloLens 裝置, 可能會造成干擾。

可能會因為這些錯誤而避免或修正。 不過, 您應該設計應用程式, 讓使用者即使在空間對應資料中出現錯誤, 也能夠達到其目標。

## <a name="the-environment-scanning-experience"></a>環境掃描體驗

HoloLens 會在使用者查看時, 瞭解其環境中的表面。 在一段時間後, HoloLens 會針對環境中所有已觀察到的部分進行掃描。 它也會在觀察到環境中的變更時, 更新掃描。 此掃描會在應用程式啟動時自動儲存。
...
* **沒有掃描經驗**
   ...
   * 例如, 讓使用者可以使用全像 spraypaint 在表面上繪製的應用程式, 只需要知道使用者目前可以看見的表面。
