...
>[!IMPORTANT]
>任何使用全像攝影遠端的應用程式, 都應該撰寫成使用[多執行緒的單元](https://docs.microsoft.com/en-us/windows/win32/com/multithreaded-apartments)。 支援使用[單一執行緒的單元](https://docs.microsoft.com/en-us/windows/win32/com/single-threaded-apartments), 但會導致在播放期間獲得最佳效能, 而且可能會間斷情形。 使用C++/WinRT [WinRT:: init_apartment](https://docs.microsoft.com/en-us/windows/uwp/cpp-and-winrt-apis/get-started)時, 多執行緒的單元是預設值。
...
若要建立從主機應用程式到 HoloLens 2 的連線, ```Connect```請在指定主機名稱和埠的遠端內容上呼叫方法。 全像攝影遠端播放程式所使用的埠是**8265**。
...
>[!TIP]
>若要避免使用[ C++/WinRT](https://docs.microsoft.com/en-us/windows/uwp/cpp-and-winrt-apis/)語言```build\native\include\<windows sdk version>\abi\Microsoft.Holographic.AppRemoting.h```投射, 可以包含位於全像的遠端 NuGet 封裝內的檔案。 它包含基礎 COM 介面的宣告。 不過, 建議C++使用/WinRT。
...
此外, 您還可以使用遠端內容上```ConnectionState```的屬性來查詢連接狀態。
```cpp
...
您可以使用非同步 helper 方法, 初始化遠端語音。 這應該會以非同步方式完成, 因為初始化可能需要相當長的時間。 [使用/WinRT 的C++並行和非同步作業](https://docs.microsoft.com/en-us/windows/uwp/cpp-and-winrt-apis/concurrency)說明如何使用C++/winrt 撰寫非同步功能
...
有兩種方式可以指定要辨識的片語。
1) 語音文法 xml 檔案中的規格。 如需詳細資訊, 請參閱[如何建立基本 XML 文法](https://docs.microsoft.com/en-us/previous-versions/office/developer/speech-technologies/hh361658(v=office.14))。
2) 藉由將字典向量內傳遞至來```ApplyParameters```指定。
...
若要在傳送至裝置```OnSendFrame```的主機應用程式中顯示相同的內容, 可以使用遠端內容的事件。 每```OnSendFrame```次全像攝影遠端程式庫將目前的框架傳送到遠端裝置時, 就會觸發此事件。 這是接受內容並將它 array.blit 到桌面或 UWP 視窗的理想時機。
