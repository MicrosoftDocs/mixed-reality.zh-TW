...
## <a name="topics-covered-in-this-tutorial"></a><span data-ttu-id="b4b33-115">在本教學課程所涵蓋的主題</span><span class="sxs-lookup"><span data-stu-id="b4b33-115">Topics covered in this tutorial</span></span>
...
* <span data-ttu-id="b4b33-158">在 [ **Inspector** ] 面板中，捲動到底部</span><span class="sxs-lookup"><span data-stu-id="b4b33-158">In the **Inspector** panel, scroll down to the bottom</span></span>
...
* <span data-ttu-id="b4b33-198">了解如何在 Unity 的遊戲模式，並在執行階段，控制器模型呈現動作。</span><span class="sxs-lookup"><span data-stu-id="b4b33-198">Learn how to render motion controller models in Unity's game mode and at runtime.</span></span>
...
<span data-ttu-id="b4b33-217">**MotionControllerVisualizer script**</span><span class="sxs-lookup"><span data-stu-id="b4b33-217">**MotionControllerVisualizer script**</span></span>
...
<span data-ttu-id="b4b33-251">**指示**</span><span class="sxs-lookup"><span data-stu-id="b4b33-251">**Instructions**</span></span>
...
<span data-ttu-id="b4b33-290">**顯示/隱藏 UI 與 Unity 的動畫控制器**</span><span class="sxs-lookup"><span data-stu-id="b4b33-290">**Showing/hiding UI with Unity's Animation controller**</span></span>
...
<span data-ttu-id="b4b33-304">在  **update （)**，這取決於**可見**屬性，就會觸發色彩選擇器的動畫元件中顯示和隱藏動畫觸發程序</span><span class="sxs-lookup"><span data-stu-id="b4b33-304">In **Update()**, based on **visible** property, it triggers Show and Hide animation triggers in the color picker's animator component</span></span>
...
## <a name="chapter-4---overriding-controller-model"></a><span data-ttu-id="b4b33-309">第 4 章-覆寫控制器模型</span><span class="sxs-lookup"><span data-stu-id="b4b33-309">Chapter 4 - Overriding controller model</span></span>
...
## <a name="chapter-5---painting-with-select-input"></a><span data-ttu-id="b4b33-329">第 5 章-使用選取的輸入繪製</span><span class="sxs-lookup"><span data-stu-id="b4b33-329">Chapter 5 - Painting with Select input</span></span>
...
<span data-ttu-id="b4b33-338">雖然**繪製**設為 true，筆刷會產生點中具現化的 Unity **LineRenderer**。</span><span class="sxs-lookup"><span data-stu-id="b4b33-338">While **Draw** is set to true, the brush will generate points in an instantiated Unity **LineRenderer**.</span></span> <span data-ttu-id="b4b33-339">此 prefab 的參考會保留在的筆刷**筆劃 Prefab**欄位。</span><span class="sxs-lookup"><span data-stu-id="b4b33-339">A reference to this prefab is kept in the brush's **Stroke Prefab** field.</span></span>
...
<span data-ttu-id="b4b33-363">針對**選取 **事件，請在**SpawnObject()**，新的物件會具現化、 非父代和發行到全球。</span><span class="sxs-lookup"><span data-stu-id="b4b33-363">For a **Select** event, in **SpawnObject()**, a new object is instantiated, un-parented and released into the world.</span></span>
...
<span data-ttu-id="b4b33-381">現在應用程式是內建，並安裝在混合實境入口網站。</span><span class="sxs-lookup"><span data-stu-id="b4b33-381">Now the app is built and installed in Mixed Reality Portal.</span></span> <span data-ttu-id="b4b33-382">您可以重新啟動透過在混合實境入口網站中的 [開始] 功能表。</span><span class="sxs-lookup"><span data-stu-id="b4b33-382">You can launch it again through Start menu in Mixed Reality Portal.</span></span>
...
<span data-ttu-id="b4b33-424">**BrushSelector 指令碼**</span><span class="sxs-lookup"><span data-stu-id="b4b33-424">**BrushSelector script**</span></span>
...
<span data-ttu-id="b4b33-441">而不是**InteractionSourcePressType.Select**中的事件**InteractionSourcePressed()**，您可以取得已按下量透過類比值**selectPressedAmount**.</span><span class="sxs-lookup"><span data-stu-id="b4b33-441">Instead of the **InteractionSourcePressType.Select** event in the **InteractionSourcePressed()**, you can get the analog value of the pressed amount through **selectPressedAmount**.</span></span> <span data-ttu-id="b4b33-442">此值可以在擷取**InteractionSourceUpdated()**。</span><span class="sxs-lookup"><span data-stu-id="b4b33-442">This value can be retrieved in **InteractionSourceUpdated()**.</span></span>
...
<span data-ttu-id="b4b33-443">**橡皮擦指令碼**</span><span class="sxs-lookup"><span data-stu-id="b4b33-443">**Eraser script**</span></span>

<span data-ttu-id="b4b33-444">**橡皮擦**是一種特殊的覆寫基底的筆刷**筆刷**的**DrawOverTime()** 函式。</span><span class="sxs-lookup"><span data-stu-id="b4b33-444">**Eraser** is a special type of brush that overrides the base **Brush**'s **DrawOverTime()** function.</span></span> <span data-ttu-id="b4b33-445">繪製，則為 true 時，橡皮擦會檢查其提示是否與任何現有的筆刷筆劃。</span><span class="sxs-lookup"><span data-stu-id="b4b33-445">While Draw is true, the eraser checks to see if its tip intersects with any existing brush strokes.</span></span> <span data-ttu-id="b4b33-446">若是如此，它們是新增至佇列，以關閉壓縮，而且刪除。</span><span class="sxs-lookup"><span data-stu-id="b4b33-446">If it does, they are added to a queue to be shrunk down and deleted.</span></span>

## <a name="advanced-design---teleportation-and-locomotion"></a><span data-ttu-id="b4b33-447">進階的設計-屏障和 locomotion</span><span class="sxs-lookup"><span data-stu-id="b4b33-447">Advanced design - Teleportation and locomotion</span></span>

<span data-ttu-id="b4b33-448">如果您想要允許使用者與使用搖桿的屏障中移動場景，請使用**MixedRealityCameraParent**而不是**MixedRealityCamera**。</span><span class="sxs-lookup"><span data-stu-id="b4b33-448">If you want to allow the user to move around the scene with teleportation using thumbstick, use **MixedRealityCameraParent** instead of **MixedRealityCamera**.</span></span> <span data-ttu-id="b4b33-449">您也需要新增**InputManager**並**DefaultCusor**。</span><span class="sxs-lookup"><span data-stu-id="b4b33-449">You also need to add **InputManager** and **DefaultCusor**.</span></span> <span data-ttu-id="b4b33-450">由於**MixedRealityCameraParent**已經包含**MotionControllers**並**界限**做為子元件，您應該移除現有**MotionControllers**並**環境**prefab。</span><span class="sxs-lookup"><span data-stu-id="b4b33-450">Since **MixedRealityCameraParent** already includes **MotionControllers** and **Boundary** as child components, you should remove existing **MotionControllers** and **Environment** prefab.</span></span>

### <a name="instructions"></a><span data-ttu-id="b4b33-451">指示</span><span class="sxs-lookup"><span data-stu-id="b4b33-451">Instructions</span></span>

* <span data-ttu-id="b4b33-452">在 [**階層**] 面板中，刪除**MixedRealityCamera**，**環境**並**MotionControllers**</span><span class="sxs-lookup"><span data-stu-id="b4b33-452">In the **Hierarchy** panel, delete **MixedRealityCamera**, **Environment** and **MotionControllers**</span></span>
* <span data-ttu-id="b4b33-453">從 **[專案] 面板**，搜尋並拖曳到下列 prefabs**階層**面板：</span><span class="sxs-lookup"><span data-stu-id="b4b33-453">From the **Project panel**, search and drag the following prefabs into the **Hierarchy** panel:</span></span>
    * <span data-ttu-id="b4b33-454">Assets/AppPrefabs/Input/Prefabs/**MixedRealityCameraParent**</span><span class="sxs-lookup"><span data-stu-id="b4b33-454">Assets/AppPrefabs/Input/Prefabs/**MixedRealityCameraParent**</span></span>
    * <span data-ttu-id="b4b33-455">Assets/AppPrefabs/Input/Prefabs/**InputManager**</span><span class="sxs-lookup"><span data-stu-id="b4b33-455">Assets/AppPrefabs/Input/Prefabs/**InputManager**</span></span>
    * <span data-ttu-id="b4b33-456">Assets/AppPrefabs/Input/Prefabs/Cursor/**DefaultCursor**</span><span class="sxs-lookup"><span data-stu-id="b4b33-456">Assets/AppPrefabs/Input/Prefabs/Cursor/**DefaultCursor**</span></span>

![混合的實境相機父代](images/mr213-cameraparent-300px.png)
* <span data-ttu-id="b4b33-458">在 **階層** 面板中，按一下 **輸入管理員**</span><span class="sxs-lookup"><span data-stu-id="b4b33-458">In the **Hierarchy** panel, click **Input Manager**</span></span>
* <span data-ttu-id="b4b33-459">在 [ **Inspector** ] 面板中，向下捲動至**簡單的單一指標選取器**區段</span><span class="sxs-lookup"><span data-stu-id="b4b33-459">In the **Inspector** panel, scroll down to the **Simple Single Pointer Selector** section</span></span>
* <span data-ttu-id="b4b33-460">從**階層** 面板中，拖曳**DefaultCursor**成**游標**欄位</span><span class="sxs-lookup"><span data-stu-id="b4b33-460">From the **Hierarchy** panel, drag **DefaultCursor** into **Cursor** field</span></span>

![指派 DefaultCursor](images/mr213-defaultcursor-500px.png)
* <span data-ttu-id="b4b33-462">**儲存**場景，然後按一下**播放** 按鈕。</span><span class="sxs-lookup"><span data-stu-id="b4b33-462">**Save** the scene and click the **play** button.</span></span> <span data-ttu-id="b4b33-463">您可以使用搖桿來旋轉左/右或傳送。</span><span class="sxs-lookup"><span data-stu-id="b4b33-463">You will be able to use the thumbstick to rotate left/right or teleport.</span></span>

## <a name="the-end"></a><span data-ttu-id="b4b33-464">結束</span><span class="sxs-lookup"><span data-stu-id="b4b33-464">The end</span></span>

<span data-ttu-id="b4b33-465">而且，本教學課程結束 ！</span><span class="sxs-lookup"><span data-stu-id="b4b33-465">And that's the end of this tutorial!</span></span> <span data-ttu-id="b4b33-466">您已了解：</span><span class="sxs-lookup"><span data-stu-id="b4b33-466">You learned:</span></span>
* <span data-ttu-id="b4b33-467">如何使用 Unity 的遊戲模式和執行階段中的動作控制器模型。</span><span class="sxs-lookup"><span data-stu-id="b4b33-467">How to work with motion controller models in Unity's game mode and runtime.</span></span>
* <span data-ttu-id="b4b33-468">如何使用不同類型的餂鈕瓿咘和他們的應用程式。</span><span class="sxs-lookup"><span data-stu-id="b4b33-468">How to use different types of button events and their applications.</span></span>
* <span data-ttu-id="b4b33-469">如何覆疊在控制器上的 UI 項目，或完全加以自訂。</span><span class="sxs-lookup"><span data-stu-id="b4b33-469">How to overlay UI elements on top of the controller or fully customize it.</span></span>

<span data-ttu-id="b4b33-470">您現在已準備好開始使用影片控制站中建立您自己的沉浸式體驗 ！</span><span class="sxs-lookup"><span data-stu-id="b4b33-470">You are now ready to start creating your own immersive experience with motion controllers!</span></span>

## <a name="completed-scenes"></a><span data-ttu-id="b4b33-471">已完成的場景</span><span class="sxs-lookup"><span data-stu-id="b4b33-471">Completed scenes</span></span>

* <span data-ttu-id="b4b33-472">在 Unity**專案**] 面板中的，按一下 [**場景**資料夾。</span><span class="sxs-lookup"><span data-stu-id="b4b33-472">In Unity's **Project** panel click on the **Scenes** folder.</span></span>
* <span data-ttu-id="b4b33-473">您會發現兩個 Unity sceens **MixedReality213**並**MixedReality213Advanced**。</span><span class="sxs-lookup"><span data-stu-id="b4b33-473">You will find two Unity sceens **MixedReality213** and **MixedReality213Advanced**.</span></span>
    * <span data-ttu-id="b4b33-474">**MixedReality213**:已完成的場景，使用單一的筆刷</span><span class="sxs-lookup"><span data-stu-id="b4b33-474">**MixedReality213**: Completed scene with single brush</span></span>
    * <span data-ttu-id="b4b33-475">**MixedReality213Advanced**:已完成的場景中填入多個筆刷選取按鈕的按下量範例</span><span class="sxs-lookup"><span data-stu-id="b4b33-475">**MixedReality213Advanced**: Completed scene with multiple brush with select button's press amount example</span></span>

## <a name="see-also"></a><span data-ttu-id="b4b33-476">另請參閱</span><span class="sxs-lookup"><span data-stu-id="b4b33-476">See also</span></span>

* [<span data-ttu-id="b4b33-477">MR 輸入 213 專案檔</span><span class="sxs-lookup"><span data-stu-id="b4b33-477">MR Input 213 project files</span></span>](https://github.com/Microsoft/MixedReality213)
* [<span data-ttu-id="b4b33-478">混合的實境工具組-動作控制器測試場景</span><span class="sxs-lookup"><span data-stu-id="b4b33-478">Mixed Reality Toolkit - Motion Controller Test scene</span></span>](https://github.com/Microsoft/MixedRealityToolkit-Unity/tree/htk_release/Assets/HoloToolkit-Examples/Input/Scenes)
* [<span data-ttu-id="b4b33-479">混合的實境工具組-抓取機制</span><span class="sxs-lookup"><span data-stu-id="b4b33-479">Mixed Reality Toolkit - Grab Mechanics</span></span>](https://github.com/Microsoft/MixedRealityToolkit-Unity/tree/htk_release/Assets/HoloToolkit-Examples/MotionControllers-GrabMechanics)
* [<span data-ttu-id="b4b33-480">動作控制器的開發指導方針</span><span class="sxs-lookup"><span data-stu-id="b4b33-480">Motion controller development guidelines</span></span>](motion-controllers.md)
