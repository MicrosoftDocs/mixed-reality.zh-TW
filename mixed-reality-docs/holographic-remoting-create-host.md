...
此外，您還可以使用遠端內容上的 ```ConnectionState``` 屬性來查詢連接狀態。
```cpp
...
您可以使用非同步 helper 方法，初始化遠端語音。 這應該會以非同步方式完成，因為初始化可能需要相當長的時間。 [使用/WinRT 的C++並行和非同步作業](https://docs.microsoft.com//windows/uwp/cpp-and-winrt-apis/concurrency)說明如何使用C++/winrt 撰寫非同步功能
...
2) 藉由在字典向量內傳遞它們來指定 ```ApplyParameters```。
