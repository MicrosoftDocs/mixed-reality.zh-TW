...
將**Grid 物件集合（腳本）** 元件新增至**3DObjectCollection**物件，並將其設定如下：
...
在 [階層] 視窗中，選取 [**乳酪**] 物件，按住**Shift**鍵，然後選取**Cube （） 2**物件，並將**操作處理常式（腳本）** 元件加入至所有物件：
...
針對**Cube （2）** 物件，變更**移動上的條件約束**以**修正與 Head 的距離**，如此一來，當物件移動時，它就會與使用者保持相同的距離：
...
若為**顆 octa**物件，請取消勾選 [**允許**最大操作] 核取方塊，讓使用者只能使用追蹤的手直接與物件互動：
...
針對**Platonic**物件，請將**一次**左右旋轉模式變更為接近，而在**一種旋轉模式**中**旋轉**，讓它在使用者手上旋轉物件時，旋轉物件中心：
...
針對**TheModule**物件，新增**Rigidbody**元件以啟用物理，然後取消核取 [**使用重心**] 核取方塊，讓物件不會受到引力的影響：

![mrlearning-base](images/mrlearning-base/tutorial4-section4-step3-7.png)

回到 [操作處理常式（腳本）] 元件，確認 [**發行行為**] 已設定為 [**保持速度**] 並**保持 [角度] 速度**，以便在物件從使用者手中釋放後，它會繼續移動：
