...
您的應用程式必須能夠使用空間對應功能。 這是必要的，因為空間網格表示使用者的環境，這可能會被視為私用資料。 在您應用程式的 package.appxmanifest.xml 檔案中宣告這項功能。 以下是範例：
...
**注意：** 如需上一個程式碼片段中使用的 CreateDirectXBuffer helper 函式，請參閱表面對應程式碼範例： SurfaceMesh .cpp、GetDataFromIBuffer。 現在裝置資源建立已完成，且網格已被視為已載入，並可供更新和呈現。
