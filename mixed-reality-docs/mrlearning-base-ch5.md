...
在 [語音輸入處理常式（腳本）] 元件上，按一下 [小型 **+** ] 按鈕以新增關鍵字，然後從 [**關鍵字**] 下拉式清單中選擇您稍早建立的 [**播放音樂**] 關鍵字：
...
在 [階層] 視窗中，**建立四個 cube**做為**PanContent**的子物件，並將其轉換**規模**設定為 X = 0.15、Y = 0.15、Z = 0.15：
...
在 [階層] 視窗中，選取 [**乳酪**] 物件，然後在 **[選取的（）** ] 事件上建立新的、設定 [**乳酪**] 物件以接收事件，並定義**EyeTrackingTutorialDemo RotateTarget**作為要觸發的動作：
