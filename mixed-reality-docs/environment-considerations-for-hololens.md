...
HoloLens 裝置知道如何藉由在空間中*追蹤*使用者, 來放置穩定且正確的全息影像。 如果沒有適當的追蹤, 裝置就不會瞭解環境或其內的使用者-因此, 全息影像可能會出現在錯誤的位置, 而不會每次出現在相同的地方, 或根本不會出現。
...
### <a name="temporal-stability-of-a-space"></a>空間的時態性穩定性
...
當您輸入新的空格 (或載入現有的空間) 時, 您會看到網格圖形散佈在空間上。 這表示您的裝置會[對應您的環境](spatial-mapping-design.md)。 

如果您在放置全息影像時遇到問題, 請試著找出空間, 讓 HoloLens 能夠更完整地對應。 
...
對應區段和不同的空間已折迭成單一資料庫, 儲存在 HoloLens 裝置的本機上。 對應資料庫會安全地儲存, 且只有內部系統可使用存取權, 而不是裝置的使用者, 即使已插入電腦及/或使用檔案瀏覽器應用程式也是如此。 啟用 bitlocker 時, 儲存的對應資料也會一併加密。

當全息影像放在不同位置時, 如果位置/全息清單之間沒有連線路徑, 則會有多個對應元件存在。  在相同的對應區段內錨定的全息影像會視為目前空間中的「附近」。

有一個開發人員 API 可匯出「目前空間」的一小部分 (這是目前已辨識的地圖元件), 以啟用共用的全息圖形案例。  目前沒有任何機制可下載所有已對應空間的整個資料庫。
