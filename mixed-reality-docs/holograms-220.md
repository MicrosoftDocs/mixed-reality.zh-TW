...
# <a name="mr-spatial-220-spatial-sound"></a>MR 空間220：空間音效
...
* 使用空間音效來增強互動。
* 將空間音效與[空間對應](spatial-mapping.md)搭配使用。
* 瞭解音效設計和混合最佳作法。
...
<td>MR 空間220：空間音效</td><td style="text-align: center;"> ✔️</td><td style="text-align: center;"> ✔️</td>
</tr>
</table>
...
注意：有一個會與您一起標記的文字面板。 這會包含可供您在本課程中使用的語音命令。
...
* **使用音效和動作來放置**：
  ...
  * **在 [取貨動作**] 和 [**放置] 動作上**，按右下方的 [+]。 將 [EnergyHub] 從場景拖曳至 [**無] （物件）** 欄位。
    ...
    * 在 **放置動作** 底下，按一下 **沒有**函式 -> **EnergyHubBase** -> **OnSelect**。
...
此設定會將 Spatialize 的數量減少到其目前層級的90%。
...
#### <a name="target-your-mix-for-70-volume-on-the-hololens"></a>將您的混合目標設為 HoloLens 上的70% 磁片區
...
磁片區層級100% 類似于虛擬實境體驗。 使用者會以視覺化方式傳輸到不同的世界。 同樣的情況也應該保留 true 語音。
...
[HoloTour](https://www.microsoft.com/store/p/holotour/9nblggh5pj87)使用身歷聲聲音。 當朗讀程式描述要查看的位置時，音效會一致，而且不會因使用者的位置而有所不同。 這可讓 [朗讀程式] 描述場景，而不需要離開環境的 hrtf 音效。
...
使用空間音效時，10-12 發射器會耗用大約12% 的 CPU。
