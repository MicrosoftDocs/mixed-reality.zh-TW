...
8.  重複相同的程式來**建立名為**[**鍵盤**] 的新標籤，並上傳適當的相片。 建立新標記之後，請務必**取消**核取*滑鼠*，以顯示 [*新增影像*] 視窗。
...
1.  使用  **資產* > 匯*入* *封裝* > *自訂* *封裝**  功能表選項，將*unitypackage*新增至 Unity。
...
4.  然後，在*CustomVisionTrainer*類別內的**Start （）** 方法上方加入下列變數。 
    ...
    > [!IMPORTANT]
    > 請確定您已新增您先前記下的**服務金鑰**（訓練金鑰）值和**專案識別碼**值;這些是您稍[早在課程中從入口網站收集的值（第2章，步驟10）](#chapter-2---training-your-custom-vision-project)。
...
1.  您將需要 HoloLens 的 IP 位址（用於遠端部署），並確保 HoloLens 處於**開發人員模式**。 做法如下：
    ...
    4.  接下來，流覽回到 [**設定**]，然後為**開發人員** **更新 & 的安全性** > 
