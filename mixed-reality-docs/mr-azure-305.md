...
## <a name="before-you-start"></a><span data-ttu-id="0b3b8-151">開始之前</span><span class="sxs-lookup"><span data-stu-id="0b3b8-151">Before you start</span></span>
...
4.  <span data-ttu-id="0b3b8-164">一旦您按下**建立**:</span><span class="sxs-lookup"><span data-stu-id="0b3b8-164">Once you have clicked on **Create**:</span></span>
    ...
    2.  <span data-ttu-id="0b3b8-166">針對*部署模型*，選取**Resource manager**。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-166">For *Deployment model*, select **Resource manager**.</span></span>
...
1.  <span data-ttu-id="0b3b8-250">開啟 Unity，然後按一下**新增**。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-250">Open Unity and click **New**.</span></span>
...
5.  <span data-ttu-id="0b3b8-265">移至**檔案 > 組建設定**並確定：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-265">Go to **File > Build Settings** and make sure that:</span></span>
    ...
    5. <span data-ttu-id="0b3b8-271">**建置並執行**設為**本機電腦**</span><span class="sxs-lookup"><span data-stu-id="0b3b8-271">**Build and Run** is set to **Local Machine**</span></span>
...
8.  <span data-ttu-id="0b3b8-284">在此窗格中，少數設定需要驗證：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-284">In this panel, a few settings need to be verified:</span></span>
    ...
    2.  <span data-ttu-id="0b3b8-289">內**發佈設定**索引標籤之下**功能**，檢查：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-289">Within the **Publishing Settings** tab, under **Capabilities**, check:</span></span>
        ...
        -  <span data-ttu-id="0b3b8-290">**InternetClient**</span><span class="sxs-lookup"><span data-stu-id="0b3b8-290">**InternetClient**</span></span>
...
3.  <span data-ttu-id="0b3b8-306">若要這樣做，請選取**齒輪**相機的旁邊的圖示*轉換*元件，然後選取**重設**。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-306">To do this, select the **Gear** icon next to the Camera's *Transform* component, and select **Reset**.</span></span>
...
2.  <span data-ttu-id="0b3b8-334">具有**平面**物件選取、 變更中的下列參數*Inspector 面板*:</span><span class="sxs-lookup"><span data-stu-id="0b3b8-334">With the **Plane** object selected, change the following parameters in the *Inspector Panel*:</span></span>
    ...
    | <span data-ttu-id="0b3b8-346">10</span><span class="sxs-lookup"><span data-stu-id="0b3b8-346">10</span></span>    | <span data-ttu-id="0b3b8-347">1</span><span class="sxs-lookup"><span data-stu-id="0b3b8-347">1</span></span>                 | <span data-ttu-id="0b3b8-348">10</span><span class="sxs-lookup"><span data-stu-id="0b3b8-348">10</span></span>    |
...
5.  <span data-ttu-id="0b3b8-371">選取新的物件，然後將它重新命名**ShapeSpawnPoint**。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-371">Select the new object and rename it **ShapeSpawnPoint**.</span></span>
...
3.  <span data-ttu-id="0b3b8-420">按一下 **匯入**按鈕以新增至您的專案項目。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-420">Click the **Import** button to add the items to your project.</span></span>
...
## <a name="chapter-7---create-the-azureservices-class"></a><span data-ttu-id="0b3b8-439">第 7-建立所需的 AzureServices 類別</span><span class="sxs-lookup"><span data-stu-id="0b3b8-439">Chapter 7 - Create the AzureServices class</span></span>
...
6.  <span data-ttu-id="0b3b8-455">加入下列 Inspector 欄位內*所需的 AzureServices*類別：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-455">Add the following Inspector Fields inside the *AzureServices* class:</span></span>
...
11. <span data-ttu-id="0b3b8-463">按一下並拖曳*所需的 AzureServices*指令碼 資料夾中的 Main Camera 物件的類別*階層面板*。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-463">Click and drag the *AzureServices* class from the Scripts folder to the Main Camera object in the *Hierarchy Panel*.</span></span>
...
5.  <span data-ttu-id="0b3b8-477">新增至如下所示的變數*ShapeFactory*類別，並取代*start （)* 並*Awake()* 與下列函式：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-477">Add the variables shown below to the *ShapeFactory* class, and replace the *Start()* and *Awake()* functions with those below:</span></span>
...
9. <span data-ttu-id="0b3b8-483">您會發現與選取的 Main Camera *ShapeFactory*遺漏指令碼元件*繁衍點*參考。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-483">With the Main Camera selected you will notice the *ShapeFactory* script component is missing the *Spawn Point* reference.</span></span> <span data-ttu-id="0b3b8-484">若要修正此問題，拖曳**ShapeSpawnPoint**物件*階層面板*來**繁衍點**參考目標。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-484">To fix it, drag the **ShapeSpawnPoint** object from the *Hierarchy Panel* to the **Spawn Point** reference target.</span></span>

    ![集合圖形 factory 參考目標](images/AzureLabs-Lab5-53.png)

## <a name="chapter-9---create-the-gaze-class"></a><span data-ttu-id="0b3b8-486">第 9-建立視線類別</span><span class="sxs-lookup"><span data-stu-id="0b3b8-486">Chapter 9 - Create the Gaze class</span></span>

<span data-ttu-id="0b3b8-487">您需要建立的最後一個指令碼*視線*類別。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-487">The last script you need to create is the *Gaze* class.</span></span>

<span data-ttu-id="0b3b8-488">這個類別會負責建立**Raycast** ，從主攝影機，來偵測使用者查看哪些物件會向前投影。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-488">This class is responsible for creating a **Raycast** that will be projected forward from the Main Camera, to detect which object the user is looking at.</span></span> <span data-ttu-id="0b3b8-489">在此情況下，進而識別是否有使用者查看需要 Raycast **GazeButton**場景中物件，並會觸發的行為。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-489">In this case, the Raycast will need to identify if the user is looking at the **GazeButton** object in the scene and trigger a behavior.</span></span>

<span data-ttu-id="0b3b8-490">若要建立此類別：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-490">To create this Class:</span></span>

1.  <span data-ttu-id="0b3b8-491">移至**指令碼**您先前建立的資料夾。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-491">Go to the **Scripts** folder you created previously.</span></span>

2.  <span data-ttu-id="0b3b8-492">在 [專案] 面板中，以滑鼠右鍵按一下**建立 >C#指令碼**。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-492">Right-click in the Project Panel, **Create > C# Script**.</span></span> <span data-ttu-id="0b3b8-493">呼叫指令碼*視線*。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-493">Call the script *Gaze*.</span></span>

3.  <span data-ttu-id="0b3b8-494">按兩下新*視線*指令碼，以開啟它*Visual Studio。*</span><span class="sxs-lookup"><span data-stu-id="0b3b8-494">Double click on the new *Gaze* script to open it with *Visual Studio.*</span></span>

4.  <span data-ttu-id="0b3b8-495">請確定下列命名空間包含在指令碼的頂端：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-495">Ensure the following namespace is included at the top of the script:</span></span>

    ```csharp
        using UnityEngine;
    ```

5.  <span data-ttu-id="0b3b8-496">然後新增下列變數內*視線*類別：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-496">Then add the following variables inside the *Gaze* class:</span></span>
...
> [!IMPORTANT]
> <span data-ttu-id="0b3b8-497">這些變數的一些能夠在中編輯*編輯器*。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-497">Some of these variables will be able to be edited in the *Editor*.</span></span>

6.  <span data-ttu-id="0b3b8-498">程式碼*Awake()* 並*start （)* 方法現在需要加入。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-498">Code for the *Awake()* and *Start()* methods now needs to be added.</span></span>
...
7.  <span data-ttu-id="0b3b8-499">新增下列程式碼會建立一個資料指標物件開頭的情況下，連同*update （)* 方法，將會執行 Raycast 方法，以及切換 GazeEnabled 布林值的位置：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-499">Add the following code, which will create a cursor object at start, along with the *Update()* method, which will run the Raycast method, along with being where the GazeEnabled boolean is toggled:</span></span>
...
8. <span data-ttu-id="0b3b8-500">接下來新增*UpdateRaycast()* 方法，它將專案 Raycast 並偵測叫用的目標。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-500">Next add the *UpdateRaycast()* method, which will project a Raycast and detect the hit target.</span></span>
...
9. <span data-ttu-id="0b3b8-501">最後，新增*ResetFocusedObject()* 方法，它會切換 GazeButton 物件目前的色彩，表示它要建立新的形狀與否。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-501">Lastly, add the *ResetFocusedObject()* method, which will toggle the GazeButton objects current color, indicating whether it is creating a new shape or not.</span></span>
...
10.  <span data-ttu-id="0b3b8-502">在 Visual Studio 中儲存的變更，然後再回到 Unity。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-502">Save your changes in Visual Studio before returning to Unity.</span></span>

11.  <span data-ttu-id="0b3b8-503">按一下並拖曳*視線*類別的指令碼資料夾**Main Camera**物件*階層面板*。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-503">Click and drag the *Gaze* class from the Scripts folder to the **Main Camera** object in the *Hierarchy Panel*.</span></span>

## <a name="chapter-10---completing-the-azureservices-class"></a><span data-ttu-id="0b3b8-504">第 10 章-完成所需的 AzureServices 類別</span><span class="sxs-lookup"><span data-stu-id="0b3b8-504">Chapter 10 - Completing the AzureServices class</span></span>

<span data-ttu-id="0b3b8-505">與其他指令碼中的地方，它現在便能夠*完整\*\*所需的 AzureServices*類別。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-505">With the other scripts in place, it is now possible to *complete* the *AzureServices* class.</span></span> <span data-ttu-id="0b3b8-506">這會透過來達成：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-506">This will be achieved through:</span></span>

1.  <span data-ttu-id="0b3b8-507">新增名為的新方法*CreateCloudIdentityAsync()*，若要設定驗證所需的變數來與 Azure 進行通訊。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-507">Adding a new method named *CreateCloudIdentityAsync()*, to set up the authentication variables needed for communicating with Azure.</span></span>

    > <span data-ttu-id="0b3b8-508">這個方法也會檢查先前儲存的檔案，包含 [圖形] 清單中存在。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-508">This method will also check for the existence of a previously stored File containing the Shape List.</span></span>
    >
    > <span data-ttu-id="0b3b8-509">**如果找到的檔案**，它會停用使用者*視線*，和形狀建立圖形，根據觸發程序，因為儲存在**Azure 儲存體檔案**。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-509">**If the file is found**, it will disable the user *Gaze*, and trigger Shape creation, according to the pattern of shapes, as stored in the **Azure Storage file**.</span></span> <span data-ttu-id="0b3b8-510">使用者可以確認這一點，作為**文字 Mesh**會提供顯示 'Storage' 或 'New'，根據圖形原點。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-510">The user can see this, as the **Text Mesh** will provide display 'Storage' or 'New', depending on the shapes origin.</span></span>
    >
    > <span data-ttu-id="0b3b8-511">**如果不找到任何檔案**，它會讓*視線*，讓使用者能夠查看時，建立圖形**GazeButton**場景中的物件。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-511">**If no file is found**, it will enable the *Gaze*, enabling the user to create shapes when looking at the **GazeButton** object in the scene.</span></span>
...
2.  <span data-ttu-id="0b3b8-512">下一步 的程式碼片段是從*start （)* 方法，其中的呼叫將會對*CreateCloudIdentityAsync()* 方法。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-512">The next code snippet is from within the *Start()* method; wherein a call will be made to the *CreateCloudIdentityAsync()* method.</span></span> <span data-ttu-id="0b3b8-513">歡迎您目前透過複製*start （)* 方法，與下面：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-513">Feel free to copy over your current *Start()* method, with the below:</span></span>
...
3.  <span data-ttu-id="0b3b8-514">方法的程式碼中填滿*CallAzureFunctionForNextShape()*。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-514">Fill in the code for the method *CallAzureFunctionForNextShape()*.</span></span> <span data-ttu-id="0b3b8-515">您將使用先前建立*Azure 函數應用程式*要求形狀索引。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-515">You will use the previously created *Azure Function App* to request a shape index.</span></span> <span data-ttu-id="0b3b8-516">一旦收到新的形狀時，這個方法會將傳送圖形，以*ShapeFactory*場景中建立新的形狀的類別。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-516">Once the new shape is received, this method will send the shape to the *ShapeFactory* class to create the new shape in the scene.</span></span> <span data-ttu-id="0b3b8-517">若要完成本文中使用下列程式碼*CallAzureFunctionForNextShape()*。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-517">Use the code below to complete the body of *CallAzureFunctionForNextShape()*.</span></span>
...
4.  <span data-ttu-id="0b3b8-518">新增方法以建立字串，串連的整數儲存圖形的歷程記錄清單，並將它儲存在您*Azure 儲存體檔案*。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-518">Add a method to create a string, by concatenating the integers stored in the shape history list, and saving it in your *Azure Storage File*.</span></span>
...
5.  <span data-ttu-id="0b3b8-519">新增方法以擷取儲存在檔案位於您*Azure 儲存體檔案*並*還原序列化*到清單。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-519">Add a method to retrieve the text stored in the file located in your *Azure Storage File* and *deserialize* it into a list.</span></span>

6.  <span data-ttu-id="0b3b8-520">完成此程序之後，此方法會重新啟用視線，讓使用者可以將多個圖形新增至場景。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-520">Once this process is completed, the method re-enables the gaze so that the user can add more shapes to the scene.</span></span>
...
7.  <span data-ttu-id="0b3b8-521">在 Visual Studio 中儲存的變更，然後再回到 Unity。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-521">Save your changes in Visual Studio before returning to Unity.</span></span>

## <a name="chapter-11---build-the-uwp-solution"></a><span data-ttu-id="0b3b8-522">第 11 章-建置 UWP 方案</span><span class="sxs-lookup"><span data-stu-id="0b3b8-522">Chapter 11 - Build the UWP Solution</span></span>

<span data-ttu-id="0b3b8-523">若要開始建置程序：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-523">To begin the Build process:</span></span>

1.  <span data-ttu-id="0b3b8-524">移至**檔案 > 組建設定**。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-524">Go to **File > Build Settings**.</span></span>

    ![建置應用程式](images/AzureLabs-Lab5-54.png)

2.  <span data-ttu-id="0b3b8-526">按一下 [建置] 。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-526">Click **Build**.</span></span> <span data-ttu-id="0b3b8-527">將會啟動 unity*檔案總管*視窗中，您要建立，然後選取 建置到應用程式的資料夾。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-527">Unity will launch a *File Explorer* window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="0b3b8-528">現在，建立該資料夾並將它命名*應用程式*。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-528">Create that folder now, and name it *App*.</span></span> <span data-ttu-id="0b3b8-529">然後使用*應用程式*資料夾選取，請按下**選取資料夾**。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-529">Then with the *App* folder selected, press **Select Folder**.</span></span>

3.  <span data-ttu-id="0b3b8-530">Unity 會開始建置您的專案*應用程式*資料夾。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-530">Unity will begin building your project to the *App* folder.</span></span>

4.  <span data-ttu-id="0b3b8-531">一次 Unity 已完成的建置 （它可能需要一些時間），它將會開啟*檔案總管*視窗在您的組建位置 （檢查您的工作列中，因為它可能不一定會出現您的視窗上方會通知您加入的新視窗）。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-531">Once Unity has finished building (it might take some time), it will open a *File Explorer* window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

## <a name="chapter-12---deploying-your-application"></a><span data-ttu-id="0b3b8-532">第 12 章-部署應用程式</span><span class="sxs-lookup"><span data-stu-id="0b3b8-532">Chapter 12 - Deploying your application</span></span>

<span data-ttu-id="0b3b8-533">若要部署您的應用程式：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-533">To deploy your application:</span></span>

1.  <span data-ttu-id="0b3b8-534">瀏覽至*應用程式*中所建立的資料夾[最後一章](#chapter-11---build-the-uwp-solution)。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-534">Navigate to the *App* folder which was created in the [last Chapter](#chapter-11---build-the-uwp-solution).</span></span> <span data-ttu-id="0b3b8-535">您會看到您的應用程式名稱，與 '.sln' 擴充功能，您應該連按兩下檔案，因此以內開啟它*Visual Studio*。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-535">You will see a file with your apps name, with the '.sln' extension, which you should double-click, so to open it within *Visual Studio*.</span></span>

2.  <span data-ttu-id="0b3b8-536">在 **的方案平台**，選取**x86，本機電腦**。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-536">In the **Solution Platform**, select **x86, Local Machine**.</span></span>

3.  <span data-ttu-id="0b3b8-537">在 **方案組態**選取**偵錯**。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-537">In the **Solution Configuration** select **Debug**.</span></span>

    > <span data-ttu-id="0b3b8-538">針對 Microsoft HoloLens，您可能會發現它更輕鬆地將此設為*遠端機器*，如此一來，您不行動網卡到您的電腦。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-538">For the Microsoft HoloLens, you may find it easier to set this to *Remote Machine*, so that you are not tethered to your computer.</span></span> <span data-ttu-id="0b3b8-539">不過，您必須也執行下列作業：</span><span class="sxs-lookup"><span data-stu-id="0b3b8-539">Though, you will need to also do the following:</span></span>
    > - <span data-ttu-id="0b3b8-540">了解**IP 位址**的您 HoloLens，位於*設定 > 網路和網際網路 > Wi-fi > 進階選項*; IPv4 是您應該使用的位址。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-540">Know the **IP Address** of your HoloLens, which can be found within the *Settings > Network & Internet > Wi-Fi > Advanced Options*; the IPv4 is the address you should use.</span></span> 
    > - <span data-ttu-id="0b3b8-541">請確定**開發人員模式**是**上**; 在找到*設定 > 更新與安全性 > 適用於開發人員*。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-541">Ensure **Developer Mode** is **On**; found in *Settings > Update & Security > For developers*.</span></span>

    ![部署解決方案](images/AzureLabs-Lab5-55.png)

4.  <span data-ttu-id="0b3b8-543">移至**建置**功能表，然後按一下 **部署方案**側載應用程式到您的電腦。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-543">Go to the **Build** menu and click on **Deploy Solution** to sideload the application to your machine.</span></span>

5.  <span data-ttu-id="0b3b8-544">您的應用程式現在應該會出現在清單中的已安裝的應用程式，即可啟動並測試 ！</span><span class="sxs-lookup"><span data-stu-id="0b3b8-544">Your App should now appear in the list of installed apps, ready to be launched and tested!</span></span>

## <a name="your-finished-azure-functions-and-storage-application"></a><span data-ttu-id="0b3b8-545">您已完成 Azure Functions 和儲存體應用程式</span><span class="sxs-lookup"><span data-stu-id="0b3b8-545">Your finished Azure Functions and Storage Application</span></span>

<span data-ttu-id="0b3b8-546">恭喜，您所建立的混合的實境應用程式，運用 Azure Functions 和 Azure 儲存體服務。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-546">Congratulations, you built a mixed reality app that leverages both the Azure Functions and Azure Storage services.</span></span> <span data-ttu-id="0b3b8-547">您的應用程式將能夠依據儲存的資料，並提供該資料為基礎的動作。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-547">Your app will be able to draw on stored data, and provide an action based on that data.</span></span>

![最終產品-結束](images/AzureLabs-Lab5-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="0b3b8-549">Bonus 練習</span><span class="sxs-lookup"><span data-stu-id="0b3b8-549">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="0b3b8-550">練習 1</span><span class="sxs-lookup"><span data-stu-id="0b3b8-550">Exercise 1</span></span>

<span data-ttu-id="0b3b8-551">建立第二個繁衍，點和記錄點已建立物件，從繁衍 （spawn）。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-551">Create a second spawn point and record which spawn point an object was created from.</span></span> <span data-ttu-id="0b3b8-552">當您載入資料檔案時，重新執行正在繁衍從原先建立的位置的圖形。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-552">When you load the data file, replay the shapes being spawned from the location they originally were created.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="0b3b8-553">練習 2</span><span class="sxs-lookup"><span data-stu-id="0b3b8-553">Exercise 2</span></span>

<span data-ttu-id="0b3b8-554">建立應用程式，而不必重新開啟它每次重新啟動的方式。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-554">Create a way to restart the app, rather than having to re-open it each time.</span></span> <span data-ttu-id="0b3b8-555">**正在載入場景**是很好的位置，以啟動。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-555">**Loading Scenes** is a good spot to start.</span></span> <span data-ttu-id="0b3b8-556">之後，若要清除的預存的清單中的方式來建立*Azure 儲存體*，如此一來，它可以輕鬆地重設您的應用程式。</span><span class="sxs-lookup"><span data-stu-id="0b3b8-556">After doing that, create a way to clear the stored list in *Azure Storage*, so that it can be easily reset from your app.</span></span> 
